...
if [ -n "$DEV_ID" ]; then
    ...
    adb -s "$DEV_ID" forward tcp:8765 tcp:8765 || true
    adb -s "$DEV_ID" forward tcp:8766 tcp:8766 || true
    adb -s "$DEV_ID" forward tcp:8767 tcp:8767 || true
    adb -s "$DEV_ID" forward tcp:8768 tcp:8768 || true
