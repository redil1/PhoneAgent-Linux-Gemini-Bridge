...
8. Package-owned conversation behavior (persisted through the persona compatibility file).
...
AgentPackage `behavior` replaces trait, communication and human-conversation settings rather than
merging the previous persona. The effective package export includes them, and edits after staging
invalidate that staged activation. Existing operator settings remain untouched until an activation.

Caller memory is namespaced by package ID, approved identity core and task objective/knowledge.
Changing business context starts a separate store; changing voice alone preserves memory. Legacy
unscoped memory is retained but never implicitly imported. Studio and the voice worker use the same
namespace, including for a shared semantic-memory database. Approved global memory blocks and
identity skills still follow their package replacement/allowlist rules.
