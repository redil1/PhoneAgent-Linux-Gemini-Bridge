...
- [ ] **M14-13 — Qualify or remove local cloud-STT corroboration.** Before the 2026-10-05
  experimental-flag expiry, evaluate independent recorded EN/FR refusals, channel choices,
  noisy onset capture, added latency and false clarification. Preserve the one-controller
  Cascade graph; keep disabled until the registered thresholds pass.
...
- [x] **M18-01 — Migrate the retired reference product.** Preserve historical regression behavior
  while removing all domain logic from shared runtime.
...
- [x] **M18-11 — Make startup neutral and MCP package-owned.** Ship no product, company, price,
  URL, or named persona in runtime defaults; require one validated Agent Package to configure
  identity, task, knowledge, tools, integrations, skills, memory, and provider settings.

**Milestone 18 exit gate**

- [x] Seven unrelated packages meet their own quality, latency, safety, and task-success gates.
- [x] No shared runtime module contains retired-product-specific policy or wording.
