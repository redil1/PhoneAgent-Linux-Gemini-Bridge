...
BACKUP = Path("/private/tmp/phoneagent-streaming-vad-before")
KEYS = (
...
def health(process, expected, *, updated):
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if updated and config.get("antigravity_live_local_corroboration") is not False:
                raise RuntimeError("Experimental local corroboration must remain disabled for this refresh")
            return {
                "local_corroboration_enabled": config.get("antigravity_live_local_corroboration", False),
                "source_sha256": {
                    name: hashlib.sha256((ROOT / name).read_bytes()).hexdigest()
                    for name in (
                        "ai_bridge/agent_policy.py",
                        "ai_bridge/cascade_tools.py",
                        "ai_bridge/conversation_repair.py",
                        "ai_bridge/human_speech.py",
                        "ai_bridge/identity/skills.py",
                        "ai_bridge/identity/store.py",
                        "ai_bridge/mcp_server.py",
                        "ai_bridge/parakeet_local_stt.py",
                        "ai_bridge/personality/persona_compiler.py",
                        "ai_bridge/production_pipeline.py",
                        "ai_bridge/runtime_config.py",
                        "ai_bridge/speech_delivery.py",
                        "ai_bridge/web_server.py",
                        "ai_bridge/antigravity_live_stt.py",
                        "ai_bridge/streaming_silero_vad.py",
                    )
                },
...
def main():
    ...
    parser.add_argument(
        "--stdout-only",
        action="store_true",
        help="Print activation evidence without rewriting the prior evidence artifact",
    )
    ...
    target = Path(__file__).with_name("runtime-streaming-vad-refresh-result.json")
