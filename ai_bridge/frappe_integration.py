...
FRAPPE_READ_ONLY_TOOLS = frozenset({
    "business_get_customer_context", "business_search_catalog", "business_get_order_status",
    "business_get_invoice_status", "business_get_support_status",
})
FRAPPE_RECEIPT_IDS = frozenset({"lead_id", "opportunity_id", "follow_up_id", "ticket_id",
                                "call_log_id", "consent_id", "quotation_id", "sales_order_id"})


def _sanitize_business_result(value: Any) -> Any:
    sanitized = _sanitize(value)
    if isinstance(value, dict) and isinstance(sanitized, dict):
        source = cast(dict[str, Any], value)
        output = cast(dict[str, Any], sanitized)
        for field in FRAPPE_RECEIPT_IDS:
            identifier = source.get(field)
            if isinstance(identifier, str) and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:-]{0,199}", identifier):
...
class FrappeConfig(BaseModel):
    ...
    @classmethod
    def _legacy_outcome_sync(cls, value: Any) -> Any:
        if isinstance(value, dict) and "record_call_outcomes_enabled" not in value:
            migrated = dict(cast(dict[str, Any], value))
            migrated["record_call_outcomes_enabled"] = migrated.get("enabled") is True
            return migrated
        return value
    ...
    enabled: bool = False
    record_call_outcomes_enabled: bool = Field(default=False, strict=True)
...
class FrappeClient:
    ...
    async def call(self, method: str, arguments: dict[str, Any] | None = None) -> Any:
        ...
        return _sanitize_business_result(body["message"])

    async def health(self) -> dict[str, Any]:
        result = await self.call("health")
        if not isinstance(result, dict) or result.get("status") != "ok":
            raise FrappeIntegrationError("Frappe PhoneAgent app is not ready")
        if result.get("required_ready") is not True:
            raise FrappeIntegrationError("Required business applications are not confirmed ready")
        return result
...
class FrappeToolRuntime:
    ...
    async def start(self) -> dict[str, RealtimeTool]:
        ...
        for name in FRAPPE_TOOL_NAMES:
            if name != "business_search_catalog" and (not self.phone_digits or not self.call_id):
                continue
    ...
    def _register_tool(self, name: str) -> None:
        ...
        spec = ToolSpec(
            ...
            read_only=name in FRAPPE_READ_ONLY_TOOLS,
        )
    ...
    async def _execute(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if name != "business_search_catalog" and (not self.phone_digits or not self.call_id):
            raise FrappeIntegrationError("A bound current caller and call ID are required")
        payload = {
            **arguments,
        ...
        if name == "business_upsert_current_lead":
            # Permission to register contact details is not permission for
            # future marketing contact or campaign enrollment.
            payload["consent_status"] = "unknown"
        ...
        return result if isinstance(result, dict) else {"verified": False, "result": result}
    ...
    def _definitions() -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "business_upsert_current_lead": function(
                ...
                "Never invent a name, email, company, consent or need. This does not grant marketing consent.",
                {
                    "name": short,
