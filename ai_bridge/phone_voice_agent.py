...
from .control_plane import reported_conversation_tuning
...
class PhoneVoiceAgent:
        ...
        while not self._stopping.is_set():
            try:
                ...
                if prewarm_ms is not None:
                    logger.info(
                        "primary LLM model ready provider=%s model=%s prewarm_ms=%.1f",
                        self.config.providers.llm_provider,
    ...
    def _call_event_sink(self, runtime: ActiveCallRuntime, caller_id: str):
        ...
        context = {
            "call_id": str(runtime.session.call_id), "caller_id": caller_id,
            "direction": self.call_direction, "channel": self.config.call_channel,
        }
        started = time.monotonic()
        def emit(event: dict[str, Any]) -> None:
            payload = {**event, **context}
            if event.get("type") == "call_outcome":
                payload["agent_duration_seconds"] = max(0.0, time.monotonic() - started)
        ...
        self._emit_event(
            {
                ...
                "config": {
                    "host_session": self.config.host_session_state(),
                    "memory_enabled": self.config.memory_enabled,
                    "credential_refs": providers.credential_refs,
                    **reported_conversation_tuning(providers),
    ...
    def _transport_params(self) -> PhoneAgentTransportParams:
        return PhoneAgentTransportParams(
            audio_in_sample_rate=self.config.sample_rate, audio_out_sample_rate=self.config.sample_rate,
            frame_ms=self.config.frame_ms, input_queue_frames=self.config.input_queue_frames,
        )
    ...
    async def _close_runtime(self, *, hangup: bool) -> None:
        ...
        if runtime.pipeline is not None:
            try:
                policy = getattr(runtime.pipeline, "policy", None)
                if policy is not None:
                    await policy.playback_disconnected()
                # This is teardown, not a request to drain speech. Draining
                # queued audio onto a closed phone caused repeated write errors
                # and a misleading final-marker failure during normal hangup.
                await runtime.pipeline.cancel("call runtime closing")
            except Exception:
        ...
        elif runtime.session.snapshot().phase is SessionPhase.CREATED:
            runtime.session.set_phase(SessionPhase.CLOSED)
