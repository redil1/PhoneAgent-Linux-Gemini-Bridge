...
from ..channel_capabilities import MESSAGE_CAPABILITIES
from ..consent import SCOPES
from ..knowledge_evidence import validate_evidence_shape
...
class TaskEngine:
    ...
    def __init__(
        ...
        self.user_contracts_dir = (
            user_contracts_dir if user_contracts_dir is not None else Path(
                os.getenv('PHONE_AGENT_TASKS_DIR', '').strip() or str(USER_CONTRACTS_DIR)
            ).expanduser()
        )
    ...
    def _validate_slots(cls, value: Any) -> list[Any]:
        ...
        for entry in value:
            if isinstance(entry, str):
                ...
                unknown = set(entry) - {"id", "question", "detect", "consent_scope"}
                if unknown:
                ...
                if 'consent_scope' in entry:
                    scope = entry['consent_scope']
                    if not isinstance(scope, str) or scope not in SCOPES | {'messaging'}:
                        raise ValueError(f'invalid consent_scope for {slot_id!r}')
                    slot['consent_scope'] = scope
    ...
    def validate_contract(cls, data: Any) -> dict[str, Any]:
        ...
        for field in cls.LIST_FIELDS:
            if field in data:
                ...
                if field == 'required_capabilities' and not set(cleaned) <= MESSAGE_CAPABILITIES:
                    raise ValueError('required_capabilities contains an unsupported capability')
                # Explicit empty allowlists must stay explicit: an absent
                # allowed_tools field can mean unrestricted legacy access.
        ...
        if 'knowledge_evidence' in data:
            contract['knowledge_evidence'] = validate_evidence_shape(data['knowledge_evidence'], contract.get('knowledge', {}))
