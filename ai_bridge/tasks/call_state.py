...
from ..consent import SLOT_SCOPES, affirmative
...
class SlotSpec:
    ...
    def parse(cls, entry: Any) -> SlotSpec:
        ...
        if isinstance(entry, str):
            return cls(id=entry.strip(), consent_scope=SLOT_SCOPES.get(entry.strip(), ''))
        ...
        return cls(
            ...
            consent_scope=str(entry.get('consent_scope') or SLOT_SCOPES.get(slot_id, '')),
        )
        ...
        *,
        ...
        for slot in self.slots:
            ...
            if slot.id in self.state:
                continue
            if slot.consent_scope:
                matched = slot.consent_scope in consent_scopes or (
                    slot.consent_scope == 'messaging' and bool(consent_scopes & {'whatsapp', 'email', 'sms'})
                )
                captured = cleaned if matched else None
            else:
                captured = slot.match(cleaned)
                # Unscoped observation regexes cannot turn a bare acknowledgement
                # into business agreement, even when the slot has a custom name.
                if captured and affirmative(captured):
                    captured = None
            if captured:
