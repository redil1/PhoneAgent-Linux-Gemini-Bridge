...
async def emit_stage_latency(sink: Any, provider: str, operation: str, started: float, **values: Any) -> None:
    """Publish metadata-only timing; diagnostic failures never break a call."""
    if sink is None:
        return
    event = {
        "type": "provider_latency", "provider": provider, "operation": operation,
    ...
    try:
        ...
        logger.debug("stage timing sink failed", exc_info=True)
