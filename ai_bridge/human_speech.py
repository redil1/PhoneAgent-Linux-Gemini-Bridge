...
_NEUTRAL = frozenset({"ok", "okay", "internet", "sport", "football", "week", "box"})
...
def _small_number_words(value: int, french: bool) -> str:
    ...
    if value < len(units):
        return units[value]
    if french and value < 20:
        return 'dix-' + units[value - 10]
    if value < 100:
        if french:
            if value >= 80:
                return 'quatre-vingts' if value == 80 else 'quatre-vingt-' + _small_number_words(value - 80, True)
            if value >= 70:
                return 'soixante et onze' if value == 71 else 'soixante-' + _small_number_words(value - 60, True)
        ...
        if unit == 0:
            return base
        if french and unit == 1:
            return base + ' et un'
        return f"{base}-{units[unit]}"
    if value < 1000:
        hundreds, rest = divmod(value, 100)
        if french:
            base = 'cent' if hundreds == 1 else units[hundreds] + (' cents' if rest == 0 else ' cent')
        else:
            base = units[hundreds] + ' hundred'
        return base + (' ' + _small_number_words(rest, french) if rest else '')
    for scale, en, fr in ((1_000_000_000, 'billion', 'milliard'), (1_000_000, 'million', 'million'), (1000, 'thousand', 'mille')):
        if scale <= value < 1_000_000_000_000:
            major, rest = divmod(value, scale)
            prefix = _small_number_words(major, french)
            if french and scale == 1000:
                prefix = prefix.removesuffix('s') if prefix.endswith(('cents', 'vingts')) else prefix
                base = 'mille' if major == 1 else prefix + ' mille'
            else:
                base = prefix + ' ' + (fr + ('s' if major > 1 else '') if french else en)
            return base + (' ' + _small_number_words(rest, french) if rest else '')
    return str(value)
...
def normalize_for_speech(text: str, language: str) -> str:
    return preserve_speech_identifiers(text, lambda prose: _normalize_speech_prose(prose, language))


def _normalize_speech_prose(text: str, language: str) -> str:
    ...
    def expand_currency(m: re.Match[str]) -> str:
        raw_amount = re.sub(r"[ \u00a0\u202f]", "", m.group("amount"))
        # Decimal groups have one/two digits; thousands groups have three.
        decimal = re.search(r"[.,](\d{1,2})$", raw_amount)
        if decimal:
            integer = re.sub(r"[.,]", "", raw_amount[:decimal.start()])
            amount_str = integer + '.' + decimal[1].ljust(2, '0')
        else:
        ...
        if "." in amount_str:
            ...
            if french:
                ...
                if cents_val > 0:
                    return f"{main_w} {unit_name} et {cents_w} centimes"
    ...
    text = re.sub(
        rf"(?<![\w.,])(?P<amount>{amount_pattern})\s*(?P<curr>€|euros?\b|EUR\b|\$|USD\b|dollars?\b|MAD\b|DH\b|dirhams?\b|£|GBP\b)",
        expand_currency,
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(
        rf"(?P<curr>€|\$|£)\s*(?P<amount>{amount_pattern})(?![\w.,]\d)",
        expand_currency,
        text,
        flags=re.IGNORECASE,
    )
