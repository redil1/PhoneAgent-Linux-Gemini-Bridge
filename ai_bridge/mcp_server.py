...
TOOLS = [
    ...
    {
        "name": "phone_agent_delete_skill",
        "description": "Delete a user-authored persona skill and revoke its trust record.",
        "inputSchema": {
            "type": "object",
...
def _call_tool(name: str, arguments: Any) -> dict[str, Any]:
    ...
    if name == "phone_agent_delete_skill":
        if set(arguments) != {"name"} or not isinstance(arguments["name"], str):
            raise LocalControlError("delete_skill needs a skill name")
        return local_control_request(
            "POST", "/api/identity/skills/delete", payload=arguments
        )
    if name in {
        ...
        # Accept the explicit envelope used by clients working around the old
        # dispatcher, while sending Studio exactly one configuration envelope.
        if set(config) == {'config'} and isinstance(config['config'], dict):
            config = config['config']
        return local_control_request(
            "POST", f"/api/{integration}", payload={'config': config}
        )
