...
class LayeredMemoryManager:
    ...
    def __init__(self, storage_path: Path | None = None, *, namespace: str = '', persistence_enabled: bool = True) -> None:
    ...
    def scoped(self, namespace: str, *, persistence_enabled: bool | None = None) -> LayeredMemoryManager:
        """Use a separate private store; never import legacy or other-scope data."""
        if len(namespace) != 64 or any(c not in '0123456789abcdef' for c in namespace):
            raise ValueError('Memory namespace must be a SHA-256 digest')
        enabled = self.persistence_enabled if persistence_enabled is None else persistence_enabled
        if self.namespace == namespace and self.persistence_enabled == enabled:
            return self
        base = self.storage_path.parent.parent.parent if self.namespace else self.storage_path.parent
        return LayeredMemoryManager(base / 'memory-scopes' / namespace / self.storage_path.name,
                                    namespace=namespace, persistence_enabled=enabled)
    ...
    def record_turn(
        ...
        *,
        ...
        generated_ai_response: str | None = None,
        delivery_status: str = "unverified",
    ) -> None:
        """Record a conversation observation with explicit playback disposition."""
        caller_id = self.normalize_caller_id(phone_number)
        with self._lock:
            ...
            mem["episodic_turns"].append(
                {
                    ...
                    "caller": caller_text,
                    "ai": ai_response,
                    "generated_ai": generated_ai_response if generated_ai_response is not None else ai_response,
