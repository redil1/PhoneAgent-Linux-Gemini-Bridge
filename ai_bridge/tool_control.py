...
class ManagedToolPolicy(BaseModel):
    ...
    read_only: bool = True
    capabilities: list[Literal['whatsapp.send', 'email.send', 'sms.send']] = Field(default_factory=list, max_length=3)

    @model_validator(mode='after')
    def _action_capabilities_require_write_access(self) -> ManagedToolPolicy:
        if self.capabilities and self.read_only:
            raise ValueError('Messaging capabilities require a writable tool')
        return self
...
class ManagedToolRuntime:
    ...
    def _register(
        ...
        self.catalog[policy.exposed_name] = RealtimeTool(
            ...
            capabilities=frozenset(policy.capabilities),
