...
class SpeculativeTurnCoordinator:
    ...
    async def consider(self, transcript: str) -> None:
        ...
        project = getattr(self._policy, "project_caller_context", None)
        if callable(project):
            speculative_context = await project(self._context, normalized)
        else:
    ...
    async def _run_candidate(
        ...
        try:
            ...
            if revision != self._revision or transcript != self._candidate:
                return
            if "<tool_call" in raw_text.casefold():
                # The cached model result can accelerate execution, but its
                # protocol is not speech. Never synthesize JSON/tool arguments
                # or hold useful TTS resources for a tool-only response.
                await self._emit(
                    "tool_ready", transcript, started,
