...
from .channel_capabilities import (
    ...
    package_messaging_requirements,
)
from .control_plane import (
    ...
    PackageValidation,
    RuntimeControl,
    conversation_tuning_env_name,
    conversation_tuning_environment,
    package_hash,
    reported_conversation_tuning,
    state_hash,
)
...
from .knowledge_evidence import evidence_schema, provenance_findings
...
from .runtime_config import ProviderConfig, RuntimeConfig, load_user_secrets
...
def _integration_config_body(data: Any) -> dict[str, Any]:
    """Accept Studio envelopes and complete configs from older MCP clients.

    Both forms still pass through the same strict integration store schema.
    Empty or ambiguous envelopes cannot reset an integration to defaults.
    """
    if not isinstance(data, dict):
        raise ValueError('config is required')
    if set(data) == {'config'} and isinstance(data['config'], dict):
        return data['config']
    if 'config' not in data and isinstance(data.get('enabled'), bool):
        return data
    raise ValueError('config is required')
...
class PhoneAgentWebServer:
    ...
    BOOLEAN_CONFIG_FIELDS = frozenset(
        {"speculative_pipeline_enabled", "conversational_reflex_enabled", "smart_turn_enabled", "antigravity_live_local_corroboration"}
    )
    ...
    def __init__(
        ...
        *,
        ...
        self.task_id = os.getenv("PHONE_AGENT_TASK_ID", "general_conversation").strip()
        ...
        self.memory_enabled = _environment_bool("PHONE_AGENT_MEMORY_ENABLED", True)
        ...
        self._websocket_shutdown_timeout_secs = 2.0
    ...
    def _setup_routes(self) -> None:
        ...
        self.app.router.add_post("/api/identity/skills/delete", self.handle_post_identity_skill_delete)
    ...
    async def handle_post_identity_skill_delete(self, request: web.Request) -> web.Response:
        try:
            data = await request.json()
            if not isinstance(data, dict) or set(data) != {"name"}:
                raise ValueError("skill name is required")
            name = str(data["name"])
            await asyncio.to_thread(self.identity_kernel.registry.delete_user_skill, name)
        except (ValueError, TypeError, SkillError) as exc:
            return web.json_response({"status": "error", "message": str(exc)}, status=400)
        await self.broadcast({"type": "identity_skill_deleted", "name": name})
    ...
    async def handle_post_product_research(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            report, activated = await build_task_from_url(
                ...
                agent_name=str(
                    data.get("agent_name")
                    or self.identity_kernel.active.core.name
                    or "PhoneAgent"
                ).strip(),
    ...
    def _active_caller_memory(self) -> LayeredMemoryManager:
        active = self.control_plane_store.active()
        scope = memory_namespace(self.identity_kernel.active, self.task_engine.require_contract(self.task_id),
                                 active.package.package_id if active else '')
        return self.memory_manager.scoped(scope)

    async def handle_get_memory(self, request: web.Request) -> web.Response:
        manager = await asyncio.to_thread(self._active_caller_memory)
        return web.json_response({'callers': manager.get_all_callers(), 'memory_scope': manager.namespace})
    ...
    async def handle_post_openwa(self, request: web.Request) -> web.Response:
        try:
            data = await request.json()
            saved = await asyncio.to_thread(self.openwa_config_store.save, _integration_config_body(data))
            await asyncio.to_thread(
                self.audit_ledger.append,
    ...
    async def handle_post_web_research(self, request: web.Request) -> web.Response:
        try:
            ...
            saved = await asyncio.to_thread(
                self.web_research_config_store.save, _integration_config_body(data)
            )
            await asyncio.to_thread(
                self.audit_ledger.append,
    ...
    async def handle_post_frappe(self, request: web.Request) -> web.Response:
        try:
            data = await request.json()
            saved = await asyncio.to_thread(self.frappe_config_store.save, _integration_config_body(data))
            await asyncio.to_thread(
                self.audit_ledger.append,
    ...
    async def handle_get_eval(self, request: web.Request) -> web.Response:
        manager = await asyncio.to_thread(self._active_caller_memory)
        return web.json_response(
            {"evaluations": manager.get_recent_evaluations(limit=20), 'memory_scope': manager.namespace}
        )
    ...
    def _public_config(self) -> dict[str, Any]:
        return {
            ...
            "credential_refs": self.config.credential_refs,
            **{name: getattr(self.config, name) for name in ConversationTuning.model_fields},
            ...
            "antigravity_live_local_corroboration": self.config.antigravity_live_local_corroboration,
            ...
            "smart_turn_enabled": self.config.smart_turn_enabled,
            "smart_turn_completion_threshold": self.config.smart_turn_completion_threshold,
            "turn_management": {
                "complete_pause_ms": self.config.antigravity_live_endpoint_ms,
                "partial_transcript_stability_ms": self.config.antigravity_live_partial_stability_ms,
                "uncertain_pause_ms": self.config.antigravity_live_fallback_endpoint_ms,
                "incomplete_pause_ms": self.config.antigravity_live_incomplete_endpoint_ms,
                "speculation_changes_speech_deadline": False,
                "acoustic_speech_detection": "silero",
            },
    ...
    def _load_saved_settings(self) -> None:
        ...
        try:
            ...
            if "smart_turn_completion_threshold" in data:
                if isinstance(data["smart_turn_completion_threshold"], bool):
                    raise ValueError("Smart Turn threshold must be a number")
                updates["smart_turn_completion_threshold"] = float(data["smart_turn_completion_threshold"])
            updates.update(self._tuning_updates(data))
            updates = self.config.provider_change_defaults(updates)
            candidate = replace(self.config, **updates)
            if "credential_refs" in updates:
                candidate = self._with_resolved_credentials(candidate)
            ...
            memory_enabled = data.get("memory_enabled", self.memory_enabled)
    ...
    @staticmethod
    def _tuning_updates(data: dict[str, Any]) -> dict[str, Any]:
        values = {key: value for key, value in data.items() if key in ConversationTuning.model_fields}
        if any(value is None for value in values.values()):
            raise ValueError('Conversation tuning fields cannot be null')
        updates = ConversationTuning.model_validate(values).model_dump(exclude_none=True)
        if "credential_refs" in data:
            updates["credential_refs"] = validate_credential_refs(data["credential_refs"])
        return updates

    @staticmethod
    def _with_resolved_credentials(config: ProviderConfig) -> ProviderConfig:
        load_user_secrets()
        return replace(config, **resolve_provider_credentials(config.credential_refs))
    ...
    async def handle_post_config(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if set(data) & LEGACY_RUNTIME_FIELDS:
                raise ValueError("These controls are retired; use the current STT, turn-taking and voice settings")
            if "record_calls" in data:
                raise ValueError("Recording must be authorized per call; record_calls is retired")
            if "parakeet_energy_threshold_dbfs" in data:
                raise ValueError("parakeet_energy_threshold_dbfs is retired: local speech uses neural turn detection")
            updates = {
                key: str(value).strip()
                for key, value in data.items()
                if key in self.CONFIG_FIELDS and str(value).strip()
    ...
    def _runtime_control(self) -> RuntimeControl:
        values = {
            ...
            if name not in {"auto_answer_enabled", "system_prompt", "memory_enabled"}
        }
        values["auto_answer_enabled"] = self.auto_answer_enabled
        values["memory_enabled"] = self.memory_enabled
    ...
    def _current_agent_package(self) -> AgentPackage:
        ...
        return AgentPackage(
            ...
            host_requirements=self._expected_host_session(),
            behavior=self.persona_compiler.conversation_behavior(),
    ...
    def _configuration_state_hash(self) -> str:
        """Optimistic-concurrency token, including revisions and private-store fingerprints."""
        return package_hash(self._current_agent_package())

    def _runtime_candidate(self, runtime: RuntimeControl) -> ProviderConfig:
        values = runtime.model_dump(
            exclude={"auto_answer_enabled", "system_prompt", "memory_enabled"}, mode="python", exclude_none=True,
        )
        candidate = replace(self.config, **values)
        if "credential_refs" in values:
            candidate = self._with_resolved_credentials(candidate)
        candidate.validate(require_credentials=False)
        return candidate
    ...
    def _validate_agent_package(self, package: AgentPackage) -> PackageValidation:
        ...
        fact_findings = provenance_findings(task)
        checks.append({'id': 'task.material_fact_evidence', 'passed': all(f['passed'] for f in fact_findings),
                       'findings': fact_findings})
        runtime_candidate = self._runtime_candidate(package.runtime)
        checks.append({"id": "runtime.compatibility", "passed": True})
        expected_host = self._expected_host_session()
        declared_host = package.host_requirements.model_dump(mode="json") if package.host_requirements else None
        mismatches = sorted(key for key in expected_host if declared_host is None or declared_host.get(key) != expected_host[key])
        checks.append({"id": "runtime.host_requirements", "passed": not mismatches,
                       "mismatched_fields": mismatches})
        required_credentials = required_provider_credentials(runtime_candidate)
        missing_credentials = sorted(field for field in required_credentials if not getattr(runtime_candidate, field))
        checks.append({"id": "runtime.credential_references", "passed": not missing_credentials,
                       "required": sorted(required_credentials), "missing": missing_credentials})
        PersonaCompiler._validate_human_conversation(package.behavior.human_conversation)
        if PersonaCompiler._identity_conflict_paths(package.behavior.communication, "communication"):
            raise ValueError("Conversation behavior conflicts with AI disclosure")
        checks.append({"id": "behavior.schema", "passed": True})
        ...
        enabled_tools = {
            ...
            if connection.enabled
            for tool in connection.tools
            if tool.enabled and assigned_to_task(tool.task_ids, task['id'])
        }
        configured_capabilities = {
            tool.exposed_name: frozenset(tool.capabilities)
            for connection in tools.connections if connection.enabled
            for tool in connection.tools
            if tool.enabled and assigned_to_task(tool.task_ids, task['id'])
        }
        if openwa.enabled:
            enabled_tools.update(tool.name for tool in openwa.tools
                                 if tool.enabled and assigned_to_task(tool.task_ids, task['id']))
        if research.enabled and assigned_to_task(research.task_ids, task['id']):
            enabled_tools.add("web_research")
        if business.enabled:
            enabled_tools.update(tool.name for tool in business.tools
                                 if tool.enabled and assigned_to_task(tool.task_ids, task['id']))
        load_user_tools()
        ...
        provided_capabilities = {
            capability for name in requested & enabled_tools
            for capability in configured_capabilities.get(name, frozenset()) | builtin_capabilities(name)
        }
        required_capabilities = package_messaging_requirements(package)
        missing_capabilities = sorted(required_capabilities - provided_capabilities)
        checks.append({
            'id': 'task.channel_capabilities', 'passed': not missing_capabilities,
            'required': sorted(required_capabilities), 'configured': sorted(provided_capabilities),
            'missing': missing_capabilities,
        })
        if missing_capabilities:
            warnings.append('Package promises or requires unavailable channel actions: ' + ', '.join(missing_capabilities))
        warnings.append('Configured bindings are not a live delivery test; verify integration readiness before use.')
    ...
    def _resolve_agent_package(self, package: AgentPackage) -> AgentPackage:
        """Capture effective optional values before hashing a staged package."""

        candidate = self._runtime_candidate(package.runtime)
        payload = package.model_dump(mode="json")
        payload["task"] = self.task_engine.validate_contract(payload["task"])
        if package.host_requirements is None:
            payload["host_requirements"] = self._expected_host_session()
        payload["runtime"].update({
            name: getattr(candidate, name) for name in ConversationTuning.model_fields
        })
        payload["runtime"]["pipeline_mode"] = candidate.pipeline_mode
        payload["runtime"]["credential_refs"] = candidate.credential_refs
        payload["runtime"]["memory_enabled"] = (
            self.memory_enabled if package.runtime.memory_enabled is None else package.runtime.memory_enabled
        )
        payload["behavior"] = PersonaCompiler.resolve_conversation_behavior(payload["behavior"])
        return AgentPackage.model_validate(payload)
    ...
    async def _activate_control_deployment(self, deployment_id: str) -> Any:
        ...
        async with self._control_activation_lock:
            ...
            if record.snapshot_version != 7:
                raise ControlPlaneError("Legacy deployment lacks resolved defaults for the current tuning schema; review and stage it again")
            current_hash = self._configuration_state_hash()
            ...
            paths = [
                self.settings_path,
                self.persona_compiler.persist_path,
                task_path,
            ...
            previous_behavior = (
                self.persona_compiler.persona_path,
                self.persona_compiler.persona_data,
                self.persona_compiler.human_conversation,
            )
            try:
                candidate = self._runtime_candidate(package.runtime)
                if package.runtime.memory_enabled is None:
                    raise ControlPlaneError("Session memory policy must be resolved before activation")
                ...
                self.memory_enabled = package.runtime.memory_enabled
                ...
                self.persona_compiler.replace_conversation_behavior(
                    package.behavior.model_dump(mode="json")
                )
                ...
                for skill_path in skill_paths:
                    if snapshot.get(skill_path) is None:
                        try:
                            ...
                            pass
                (self.config, self.task_id, self.system_prompt,
                 self.auto_answer_enabled, self.memory_enabled) = previous
                (
                    self.persona_compiler.persona_path,
                    self.persona_compiler.persona_data,
                    self.persona_compiler.human_conversation,
        ...
        return web.json_response(
            {
                ...
                'task_extensions': {
                    'knowledge_evidence': evidence_schema(),
                    'slot_consent_scope': {'location': 'task.inputs_required[].consent_scope',
                                           'enum': ['continue', 'interest', 'budget', 'whatsapp', 'email', 'sms', 'messaging', 'purchase', 'registration']},
                    'required_capabilities': {'type': 'array', 'items': {'enum': ['whatsapp.send', 'email.send', 'sms.send']}},
                },
        ...
        return web.json_response(
            {
                ...
                "effective_state_hash": state_hash(package.model_dump(mode="json")),
                "configuration_state_hash": package_hash(package),
    ...
    async def handle_post_control_validate(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            package = self._resolve_agent_package(AgentPackage.model_validate(data["package"]))
        ...
        return web.json_response(
            {"status": "ok", "validation": validation.model_dump(mode="json"),
             "resolved_runtime": package.runtime.model_dump(mode="json"),
             "resolved_behavior": package.behavior.model_dump(mode="json")}
        )
    ...
    async def handle_post_control_stage(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            record = self.control_plane_store.stage(
                package,
                validation,
                base_state_hash=self._configuration_state_hash(),
                reason=str(data["reason"]),
    ...
    async def handle_post_control_rollback(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if source.snapshot_version != 7:
                raise ControlPlaneError(
                    "Exact rollback is unavailable for this legacy deployment: defaults were not recorded. "
                    "Review its package and stage a new resolved deployment."
                )
            validation = self._validate_agent_package(source.package)
            if not validation.valid:
                failed = ", ".join(str(check["id"]) for check in validation.checks if not check["passed"])
                raise ControlPlaneError("Rollback package no longer validates: " + failed)
    ...
    def _child_environment(
        ...
        *,
        ...
        env = os.environ.copy()
        for key in list(env):
            if key.startswith("PHONE_AGENT_CHATGPT_"):
                env.pop(key)
        env.pop("PHONE_AGENT_RECORD_CALLS", None)
        if command_stdin:
        ...
        env.update(
            {
                ...
                "PHONE_AGENT_ANTIGRAVITY_ENDPOINT_MS": str(self.config.antigravity_live_endpoint_ms),
                "PHONE_AGENT_ANTIGRAVITY_FALLBACK_ENDPOINT_MS": str(self.config.antigravity_live_fallback_endpoint_ms),
                "PHONE_AGENT_ANTIGRAVITY_INCOMPLETE_ENDPOINT_MS": str(self.config.antigravity_live_incomplete_endpoint_ms),
                "PHONE_AGENT_ANTIGRAVITY_STABILITY_MS": str(self.config.antigravity_live_stability_ms),
                "PHONE_AGENT_ANTIGRAVITY_LOCAL_CORROBORATION": str(self.config.antigravity_live_local_corroboration).lower(),
                "PHONE_AGENT_ANTIGRAVITY_PARTIAL_STABILITY_MS": str(self.config.antigravity_live_partial_stability_ms),
                "PHONE_AGENT_SPECULATIVE_FAST_ENDPOINT_MS": str(self.config.speculative_fast_endpoint_ms),
                "PHONE_AGENT_SPECULATIVE_AMBIGUOUS_ENDPOINT_MS": str(self.config.speculative_ambiguous_endpoint_ms),
                "PHONE_AGENT_SPECULATIVE_INCOMPLETE_ENDPOINT_MS": str(self.config.speculative_incomplete_endpoint_ms),
                "PHONE_AGENT_SMART_TURN_ENABLED": "true" if self.config.smart_turn_enabled else "false",
                "PHONE_AGENT_SMART_TURN_COMPLETION_THRESHOLD": str(self.config.smart_turn_completion_threshold),
                "PHONE_AGENT_CONVERSATION_REPAIR": "true" if self.config.conversation_repair_enabled else "false",
                "PHONE_AGENT_EDGE_TTS_RATE": self.config.edge_tts_rate,
                'PHONE_AGENT_EDGE_TTS_VOLUME': self.config.edge_tts_volume,
                'PHONE_AGENT_EDGE_TTS_PITCH': self.config.edge_tts_pitch,
                ...
                "PHONE_AGENT_MEMORY_ENABLED": str(self.memory_enabled).lower(),
                'PHONE_AGENT_CONTROL_PLANE_ROOT': str(self.control_plane_store.root),
                'PHONE_AGENT_TASKS_DIR': str(self.task_engine.user_contracts_dir),
        ...
        env.update(conversation_tuning_environment(self.config))
        env["PHONE_AGENT_PROVIDER_CREDENTIAL_REFS"] = json.dumps(self.config.credential_refs, sort_keys=True)
        return env
    ...
    def _voice_host_environment_signature(
        ...
        tuning_names = {conversation_tuning_env_name(field) for field in ConversationTuning.model_fields}
        return tuple(
            sorted(
                (name, value)
                for name, value in environment.items()
                if name.startswith("PHONE_AGENT_") or name in tuning_names
            )
        )

    def _expected_host_session(self) -> dict[str, Any]:
        environment = self._child_environment(auto_answer=self.auto_answer_enabled,
                                              recording_consent=False, call_channel="gsm", command_stdin=True)
        return RuntimeConfig.from_session_environment(environment, providers=self.config).host_session_state()

    def _expected_voice_host_config(self) -> dict[str, Any]:
        return {
            "host_session": self._expected_host_session(),
            "memory_enabled": self.memory_enabled,
            "credential_refs": self.config.credential_refs,
            **reported_conversation_tuning(self.config),
            "pipeline_mode": self.config.pipeline_mode,
            "stt_provider": self.config.stt_provider,
            "stt_model": self.config.stt_model,
            "stt_language": self.config.stt_language,
    ...
    def _resident_host_matches_current_config(self) -> bool:
        if not self._resident_host_ready:
            return False
        try:
            return bool(
                self._resident_host_reported_config == self._expected_voice_host_config()
    ...
    async def _handle_child_line(self, line: str) -> None:
            ...
            if isinstance(event, dict):
                if event.get("type") == "voice_host_ready":
                    ...
                    try:
                        ...
                        logger.error("Host session settings are invalid; refusing worker readiness")
                        expected = None
        ...
        if state_match:
            ...
            for marker in (
                ...
                "Caller turn committed while bot speaking",
    ...
    async def _close_websocket(self, ws: web.WebSocketResponse) -> None:
        try:
            await asyncio.wait_for(
                ws.close(code=WSCloseCode.GOING_AWAY, message=b"Studio shutting down"),
                timeout=self._websocket_shutdown_timeout_secs,
            )
        except TimeoutError:
            # aiohttp closes the transport when its closing handshake is
            # cancelled. A non-responsive browser cannot hold shutdown open.
            logger.warning("Timed out closing a Studio WebSocket")
        except Exception:
            logger.warning("Failed to close a Studio WebSocket", exc_info=True)

    async def handle_websocket(self, request: web.Request) -> web.WebSocketResponse:
        if self._shutting_down:
            raise web.HTTPServiceUnavailable(text="Studio is shutting down")
    ...
    async def _sync_frappe_call_outcome(self, event: dict[str, Any]) -> None:
        try:
            config = await asyncio.to_thread(self.frappe_config_store.load)
            if not config.enabled or not config.record_call_outcomes_enabled:
                return
            ...
            caller = _phone_digits(str(event.get("caller_id") or ""))
            call_id = str(event.get("call_id") or "").strip()
            if not caller or not call_id:
                logger.warning("Skipped outcome sync without immutable caller/call identity")
                return
            duration = max(0.0, float(event.get("agent_duration_seconds") or 0))
            async with aiohttp.ClientSession() as session:
                await FrappeClient(config, session).call(
                    "record_call_outcome",
                    {
    ...
    async def _on_shutdown(self, app: web.Application) -> None:
        ...
        if clients:
            await asyncio.gather(*(self._close_websocket(ws) for ws in clients))
        if self._remote_link is not None:
            await self._remote_link.close()
            self._remote_link = None
        await self._stop_inbound_monitor()
