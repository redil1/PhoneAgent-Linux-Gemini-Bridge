...
from ..identity.models import LanguageCode
from ..knowledge_evidence import provenance_findings
...
COMPILED_TRAIT_DEFAULTS = {
    "analytical": 0.95, "direct": 0.90, "tolerant_of_vagueness": 0.10,
    "empathetic": 0.80, "persuasive": 0.80,
}

_IDENTITY_DECEPTION_PATTERNS = (
    re.compile(r"\b(?:pretend|claim|say|confirm|present|portray)\b.{0,80}\b(?:real\s+)?human\b", re.I),
    re.compile(r"\b(?:never|do\s+not|don'?t)\s+(?:say|admit|disclose|mention|reveal)\b.{0,80}\b(?:ai|artificial intelligence|bot|virtual assistant|language model)\b", re.I),
    re.compile(r"\b(?:deny|hide|conceal)\b.{0,60}\b(?:ai|artificial intelligence|bot|virtual assistant)\b", re.I),
    re.compile(r"\blaugh\b.{0,100}\b(?:ai|robot|bot)\b.{0,100}\b(?:real|human|consultant|advisor)\b", re.I),
    re.compile(r"\bif\s+asked\b.{0,100}\b(?:ai|robot|bot)\b.{0,120}\b(?:real|human|consultant|advisor)\b", re.I),
    re.compile(r"\b(?:real\s+human|human\s+persona|impersonat(?:e|ing)\s+(?:a\s+)?human)\b", re.I),
)


def _deceptive_identity_instruction(value: str) -> bool:
    """Identify instructions that conflict with the immutable AI disclosure."""

    normalized = " ".join(str(value).split())
    return any(pattern.search(normalized) for pattern in _IDENTITY_DECEPTION_PATTERNS)
...
class PersonaCompiler:
    ...
    def __init__(
        ...
        *,
        ...
        self._dropped_identity_conflicts = 0
        self.human_conversation = self._sanitize_identity_conflicts(self.human_conversation)
    ...
    def _load_persona(self) -> dict[str, Any]:
        if not self.persona_path.exists():
            return {
                "identity": {"name": "PhoneAgent", "role": "AI phone representative"},
    ...
    def _load_human_conversation(self) -> dict[str, Any]:
        ...
        if self.persona_data.get("defaults_resolved") is True:
            return copy.deepcopy(override)
        return self._conversation_with_defaults(override)

    @classmethod
    def _conversation_with_defaults(cls, override: Any) -> dict[str, Any]:
            ...
            for section, value in override.items():
                if isinstance(value, dict) and isinstance(defaults.get(section), dict):
                    ...
                    for key, inner in value.items():
                        merged[key] = cls._merge_wordings(merged.get(key), inner)
                    defaults[section] = merged
                else:
                    defaults[section] = cls._merge_wordings(defaults.get(section), value)
        return defaults

    def _sanitize_identity_conflicts(self, value: Any) -> Any:
        """Drop legacy prompt lines that contradict the approved identity core.

        The persisted persona remains untouched so Studio can show the operator
        what was authored. Only the effective runtime behavior is filtered.
        """

        if isinstance(value, str):
            if _deceptive_identity_instruction(value):
                self._dropped_identity_conflicts += 1
                logger.error("Ignored persona instruction conflicting with AI disclosure")
                return ""
            return value
        if isinstance(value, list):
            return [
                cleaned
                for item in value
                if (cleaned := self._sanitize_identity_conflicts(item)) is not None
                and cleaned != ""
            ]
    ...
    def repair_phrases(self, language: str | None = None) -> dict[str, Any]:
        ...
        language_code = LanguageCode.FR if code.lower().startswith("fr") else LanguageCode.EN
        disclosure = self.identity_kernel.active.core.ai_disclosure.get(
            language_code,
            self.identity_kernel.active.core.ai_disclosure.get(LanguageCode.EN, ""),
        ).strip()
        return {
            ...
            "identity": [disclosure] if disclosure else self._for_language(
                situations.get("asked_who_you_are"), code
            ),
        }
    ...
    @staticmethod
    def _whatsapp_action_guidance(connected: set[str]) -> str:
        if "whatsapp_send_text_current_customer" not in connected:
            return (
                "- WhatsApp sending is unavailable in this call. Do not offer or promise it. "
                "A caller number does not grant messaging capability. Offer another messaging "
                "channel only if its sending tool is actually connected."
            )
        return (
            "- When the caller requests an offer or checkout link and approves your proposal, "
            "execute whatsapp_send_text_current_customer with the verified details. A clear yes "
            "to your immediately preceding send proposal is authorization; do not ask for the "
            "same consent again. Say WhatsApp, not SMS. If the plan is genuinely unclear, "
            "clarify it once or send the requested comparison. Do not deny this capability "
            "without an actual tool failure. Do not read a checkout URL aloud unless asked."
        )
        ...
        *,
        ...
        parts = [
            ...
            (
                ...
                "- State the caller number only if they ask for it. Send messages only "
                "through an actually connected, caller-bound tool after the caller requests "
                "or approves the action. Having their number is not messaging capability."
                if current_number
                else ""
            ),
            "",
        ...
        if task_contract:
            ...
            unreviewed = [finding['fact'] for finding in provenance_findings(task_contract) if not finding['passed']]
            if unreviewed:
                verified_facts = (
                    'UNREVIEWED CONFIGURATION: ' + ', '.join(unreviewed) + '. '
                    'These entries lack current source review. They are not verified facts; '
                    'do not quote their prices, certifications, guarantees or other material claims.\n'
                    + verified_facts
                )
            ...
            parts.extend(
                [
                    ...
                    self._whatsapp_action_guidance(set(connected_tools)),
    ...
    def compile_realtime(
        ...
        *,
        ...
        name = str(identity.get("name", "PhoneAgent")).strip()
        ...
        if not connected:
            ...
            rules.append(self._whatsapp_action_guidance(set(connected)))
            if business_tools:
                rules.append(
...
"""
    ...
    def _validate_human_conversation(cls, value: Any) -> dict[str, Any]:
        ...
        prompt_sections = {key: item for key, item in validated.items() if key != "examples"}
        conflict_paths = cls._identity_conflict_paths(prompt_sections)
        if conflict_paths:
            raise ValueError(
                "human_conversation conflicts with the immutable AI disclosure: "
                + ", ".join(conflict_paths)
            )
        return validated

    @classmethod
    def _identity_conflict_paths(cls, value: Any, path: str = "human_conversation") -> list[str]:
        if isinstance(value, str):
            return [path] if _deceptive_identity_instruction(value) else []
        if isinstance(value, list):
            return [
                conflict
                for index, item in enumerate(value)
                for conflict in cls._identity_conflict_paths(item, f"{path}[{index}]")
            ]
        if isinstance(value, dict):
            return [
                conflict
                for key, item in value.items()
                for conflict in cls._identity_conflict_paths(item, f"{path}.{key}")
            ]
        return []
    ...
    def conversation_behavior(self) -> dict[str, Any]:
        """Export effective behavior, including the shipped conversation defaults."""

        return self.resolve_conversation_behavior({
            "defaults_resolved": self.persona_data.get("defaults_resolved", False),
            "trait_intensity": self.persona_data.get("trait_intensity", {}),
            "communication": self.persona_data.get("communication", {}),
            "human_conversation": self.human_conversation,
        })

    @classmethod
    def resolve_conversation_behavior(cls, behavior: dict[str, Any]) -> dict[str, Any]:
        """Freeze inherited defaults once, before package validation and staging."""

        result = copy.deepcopy(behavior)
        conversation = result.get("human_conversation", {})
        if not result.get("defaults_resolved"):
            conversation = cls._conversation_with_defaults(conversation)
            result["trait_intensity"] = {**COMPILED_TRAIT_DEFAULTS, **result.get("trait_intensity", {})}
            result["communication"] = {"default_style": "natural and direct", **result.get("communication", {})}
        result["human_conversation"] = cls._validate_human_conversation(conversation)
        result["defaults_resolved"] = True
        return result

    def replace_conversation_behavior(self, behavior: dict[str, Any]) -> None:
        """Replace package behavior rather than merging the previous business's wording."""

        data = self.resolve_conversation_behavior(behavior)
        data["identity"] = self.effective_identity
        atomic_write_private(
            self.persist_path, yaml.safe_dump(data, allow_unicode=True, sort_keys=False)
        )
        self.persona_path = self.persist_path
        self.persona_data = data
        self.human_conversation = self._sanitize_identity_conflicts(self._load_human_conversation())
