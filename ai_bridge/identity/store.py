...
def default_profile_from_legacy(
    ...
    name = str(identity.get("name") or "PhoneAgent")
...
class IdentityStore:
    ...
    def create_revision(
        ...
        with self._lock:
            ...
            same_identity = candidate.identity_id == active.identity_id
            candidate = candidate.model_copy(
                update={
                    "version": max(known_versions) + 1,
                    "created_at": active.created_at if same_identity else utc_now(),
                    "updated_at": utc_now(),
                }
            )
