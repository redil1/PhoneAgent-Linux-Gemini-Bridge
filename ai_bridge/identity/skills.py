...
class SkillRegistry:
    ...
    def delete_user_skill(self, name: str) -> None:
        """Delete one user-authored skill and revoke its stored trust record."""

        if not SKILL_ID_RE.fullmatch(name):
            raise SkillError("skill id is invalid")
        if (BUILTIN_SKILLS / name).exists():
            raise SkillError("built-in skills cannot be deleted")
        directory = self.user_root / name
        if directory.is_symlink() or (directory.exists() and not directory.is_dir()):
            raise SkillError("skill directory must be a regular directory")
        with self._lock:
            if not directory.exists():
                raise SkillError("user skill does not exist")
