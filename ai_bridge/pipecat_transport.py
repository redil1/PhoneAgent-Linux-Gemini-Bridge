...
from .speech_floor_guard import SPEECH_RESPONSE_ID
...
class PhoneAgentOutputTransport(BaseOutputTransport):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._initialized = False
        self._tx_accumulator = bytearray()
        self._audio_since_last_end = False
        self._audio_end_epoch = 0
        self._queued_response_id: str | None = None
        self._speaking_response_id: str | None = None
    ...
    async def write_audio_frame_result(self, frame: OutputAudioRawFrame) -> AudioWriteResult:
        ...
        self._tx_accumulator.extend(payload)
        chunk_size = self._phone_params.input_frame_bytes  # Exactly 640 bytes required by Android

        while len(self._tx_accumulator) >= chunk_size:
            chunk = bytes(self._tx_accumulator[:chunk_size])
            del self._tx_accumulator[:chunk_size]
    ...
    async def push_frame(self, frame: Frame, direction: FrameDirection = FrameDirection.DOWNSTREAM) -> None:
        # This hook runs at emission from the ordered media sender. Sampling
        # mutable ownership later in an observer can assign a delayed event
        # to the next response instead.
        if isinstance(frame, BotStartedSpeakingFrame):
            if direction is FrameDirection.DOWNSTREAM:
                self._speaking_response_id = self._queued_response_id
    ...
    async def write_transport_frame(self, frame: Frame) -> None:
        if isinstance(frame, TTSStartedFrame):
            ...
            self._queued_response_id = response_id if isinstance(response_id, str) else None
    ...
    async def finish_audio_segment(self) -> tuple[int, int] | None:
        ...
        """
        chunk_size = self._phone_params.input_frame_bytes
        if self._tx_accumulator:
            remainder = bytes(self._tx_accumulator)
            self._tx_accumulator.clear()
            chunk = remainder + b"\x00" * (chunk_size - len(remainder))
            try:
                generation_id, sequence = self._session.next_output_identity()
                if generation_id == self._session.generation_id and self._tx_handler is not None:
                    await _invoke_callback(self._tx_handler, chunk, generation_id, sequence)
                    self._session.account_output(generation_id, sequence, len(chunk))
                    self._transport.notify_output_audio(chunk)
                    self._audio_since_last_end = True
            except Exception as exc:
                logger.warning("phone trailing frame write failed: %s", exc)
