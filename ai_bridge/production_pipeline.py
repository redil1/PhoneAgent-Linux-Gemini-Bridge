...
from .action_receipts import DEFAULT_JOURNAL_PATH, ActionJournal
...
from .committed_turn_strategy import CommittedTranscriptStopStrategy
from .conversational_reflex import ConversationalReflexProcessor
from .generation_recovery import bind_generation_failures
from .pipecat_transport import PhoneAgentTransport
from .repair_processor import ConversationRepairProcessor, UncertainTurnGate
...
def _local_whisper_model(config: ProviderConfig) -> str:
    ...
    if config.stt_provider == "whisper_mlx":
        from .mlx_whisper_decoder import DEFAULT_MLX_WHISPER_MODEL
        if "mlx-community" in model_name and "whisper" in model_name:
            return model_name
        if model_name in {"", "large-v3-turbo", "gemini-3.1-flash-live-preview"}:
            return DEFAULT_MLX_WHISPER_MODEL
        raise ValueError("whisper_mlx requires an MLX Whisper checkpoint")
...
def _create_local_turn_service(config: ProviderConfig, sample_rate: int, model: str) -> Any:
    """Use neural acoustic ownership for every buffered Whisper/Parakeet path."""
    from .local_neural_stt import LocalNeuralSTTService

    return LocalNeuralSTTService(
        model=model, sample_rate=sample_rate,
        language=config.stt_language, decode_language="auto",
        silence_endpoint_ms=(config.antigravity_live_endpoint_ms if config.stt_provider == "whisper_mlx" else config.parakeet_endpoint_ms),
        incomplete_endpoint_ms=(config.antigravity_live_incomplete_endpoint_ms if config.stt_provider == "whisper_mlx" else config.parakeet_incomplete_endpoint_ms),
        ...
        smart_turn_enabled=config.smart_turn_enabled,
        smart_turn_model_path=config.smart_turn_model_path,
        smart_turn_completion_threshold=config.smart_turn_completion_threshold,
...
def create_provider_services(config: ProviderConfig, sample_rate: int) -> ProviderServices:
    ...
    if config.stt_provider == "deepgram_flux":
        ...
        stt = _create_local_turn_service(config, sample_rate, _local_whisper_model(config))
        ...
        stt_class = AntigravityLiveSTTService
        if config.antigravity_live_local_corroboration:
            from .corroborated_cloud_stt import CorroboratedCloudSTTService
            stt_class = CorroboratedCloudSTTService
        stt = stt_class(
            sample_rate=sample_rate,
            ...
            transcript_stability_ms=config.antigravity_live_stability_ms,
            partial_transcript_stability_ms=config.antigravity_live_partial_stability_ms,
            fallback_endpoint_ms=config.antigravity_live_fallback_endpoint_ms,
            speculative_pipeline_enabled=config.speculative_pipeline_enabled,
            speculative_prefetch_silence_ms=config.speculative_prefetch_silence_ms,
            speculative_prefetch_stability_ms=config.speculative_prefetch_stability_ms,
            ...
            smart_turn_enabled=config.smart_turn_enabled,
            smart_turn_model_path=config.smart_turn_model_path or None,
            smart_turn_completion_threshold=config.smart_turn_completion_threshold,
        )
        ...
        if not config.stt_language.lower().startswith("en"):
            ...
            stt = _create_local_turn_service(config, sample_rate, "large-v3-turbo")
        else:
        ...
        stt = _create_local_turn_service(config, sample_rate, config.stt_model)
    else:
    ...
    if config.tts_provider == "supertonic":
        ...
        google_tts_key = config.google_api_key
...
async def prewarm_primary_llm(config: ProviderConfig) -> float | None:
    """Make the selected primary LLM ready before the host accepts calls."""

    if config.llm_provider == "antigravity_gemini":
        from .antigravity_gemini_llm import AntigravityGeminiLLMService

        svc = AntigravityGeminiLLMService(model=config.llm_model)
        try:
            started = asyncio.get_running_loop().time()
            ok = await svc._discover_bridge()
            if ok:
                elapsed_ms = (asyncio.get_running_loop().time() - started) * 1000
                logger.info(
                    "Antigravity Gemini LLM bridge verified model=%s latency=%.1fms",
                    config.llm_model,
                    elapsed_ms,
                )
                return elapsed_ms
        except Exception as exc:
            logger.debug("Antigravity bridge check during prewarm: %s", exc)
        return None
...
async def prewarm_speech_models(config: ProviderConfig) -> dict[str, float]:
    ...
    if needs_whisper:
        try:
            ...
            raise RuntimeError(
                f"Selected local STT could not be prepared ({type(exc).__name__}); "
                "install the local STT dependencies and model before dialing"
            ) from exc
    if (
...
class ProductionCallPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self.policy = AgentPolicyRuntime(
            ...
            event_sink=self._on_policy_event,
        )
        for service in (self.services.stt, self.services.llm, self.services.tts):
        ...
        self.tools = CascadeToolRuntime(
            ...
            action_journal=ActionJournal(DEFAULT_JOURNAL_PATH),
        )
        self._tool_protocol_message: dict[str, str] | None = None
        ...
        self._generation_recovery_epochs: set[int] = set()
        self._uncertain_recovery_epochs: set[int] = set()
        self.uncertain_turn_gate = UncertainTurnGate(self.policy, self._recover_uncertain_transcription)
        async def recover_output(frame, epoch):
            if frame.processor is self.services.tts and SYNTHESIS_CONTEXT_ID in frame.metadata:
                await self._recover_generation_failure(epoch, provider="tts")
        self.response_policy.bind_output_recovery(recover_output)
        if getattr(self.services.llm, "terminal_generation_errors", False):
            bind_generation_failures(self.services.llm, lambda: self.policy.turn_epoch)
            self.response_policy.bind_generation_recovery(self._recover_generation_failure)
        self.speech_floor_guard: SpeechFloorGuard | None = None
        caller_floor = getattr(self.services.stt, "caller_owns_floor", None)
        if callable(caller_floor):
            self.response_policy.enable_speech_floor_guard()
            self.speech_floor_guard = SpeechFloorGuard(
                caller_owns_floor=caller_floor,
                turn_epoch=lambda: self.policy.turn_epoch,
            )
            self._last_turn_recovery_at = 0.0
            self.services.stt.set_turn_recovery_handler(self._recover_untranscribed_turn)
        ...
        } or bool(getattr(self.services.stt, "commits_transcripts_externally", False))
        external_strategies = ExternalUserTurnStrategies() if uses_external_turn_frames else None
        if config.providers.stt_provider in {"antigravity_live", "parakeet_local", *LOCAL_WHISPER_PROVIDERS} or getattr(self.services.stt, "commits_transcripts_externally", False):
            external_strategies.stop = [CommittedTranscriptStopStrategy()]
        self.user_aggregator, self.assistant_aggregator = LLMContextAggregatorPair(
            ...
            user_params=LLMUserAggregatorParams(
                ...
                user_turn_stop_timeout=float(getattr(self.services.stt, "turn_stop_watchdog_timeout_secs", 5.0)),
            ),
        )
        ...
        self.pipeline = Pipeline(
            [
                ...
                for element in (
                    ...
                    self.uncertain_turn_gate,
                    self.conversational_reflex,
                    self.services.llm,
                    self.tool_processor,
                    self.response_policy,
                    self.services.tts,
                    self.speech_floor_guard,
                    transport.output(),
    ...
    async def _on_policy_event(self, event: dict[str, Any]) -> None:
        if event.get("type") == "terminal_completion_aborted":
            # A caller may continue during the goodbye. A later model-owned
            # end_call must still be possible after the conversation resumes.
            self._terminal_request_accepted = False
        await self._emit(event)

    async def _accept_ai_end_call(self, result: dict[str, Any], *, expected_epoch: int | None = None) -> None:
        """Speak one model-selected close and hang up only after verified playout."""

        async with self._terminal_request_lock:
            if expected_epoch is not None and self.policy.is_stale(expected_epoch):
                return
            epoch = self.policy.turn_epoch
            ...
            if not spoken or self.policy.is_stale(epoch):
    ...
    async def _recover_generation_failure(self, epoch: int, *, provider: str = "llm") -> None:
        """Offer one honest repair, without replaying a possibly completed tool."""
        floor = getattr(self.services.stt, "caller_owns_floor", None)
        if (self.policy.is_stale(epoch) or not self.transport.session.is_active
                or (callable(floor) and floor()) or epoch in self._generation_recovery_epochs
                or not self.policy.last_caller_text):
            return
        if provider == "tts" and self.policy.has_pending_speech():
            await self._emit({"type": "provider_recovery_skipped", "provider": "tts",
                              "turn_epoch": epoch, "reason": "valid_speech_already_queued"})
            return
        self._generation_recovery_epochs.add(epoch)
        self._generation_recovery_epochs = {value for value in self._generation_recovery_epochs if value >= epoch - 64}
        french = self.policy.reply_language.lower().startswith("fr")
        terminal = self.policy.terminal_control_kind(self.policy.last_caller_text) if self.policy.last_caller_transcript_trusted else None
        if terminal:
            await self._accept_ai_end_call({
                "reason": "caller ended the conversation during a generation failure",
                "closing_message": "Merci pour votre temps. Au revoir." if french else "Thanks for your time. Goodbye.",
            }, expected_epoch=epoch)
            return
        text = (
            "Désolé, je n'ai pas pu terminer ma réponse. Pouvez-vous réessayer ?"
            if french else "Sorry, I couldn't finish that response. Please try again."
        )
        spoken, _, response_id = await self.policy.finalize_response_with_identity(text, response_kind="provider_recovery")
        if not spoken or self.policy.is_stale(epoch) or not self.transport.session.is_active:
            self.policy.discard_pending_playback(response_id)
            return
        frame = TTSSpeakFrame(spoken, append_to_context=True)
        frame.metadata[SPEECH_TURN_EPOCH] = epoch
        ...
        await self._emit({"type": "provider_recovery", "provider": provider, "turn_epoch": epoch,
                          "response_id": response_id, "automatic_tool_retry": False})

    async def _recover_uncertain_transcription(self, epoch: int) -> None:
        floor = getattr(self.services.stt, "caller_owns_floor", None)
        if (self.policy.is_stale(epoch) or self.policy.last_caller_transcript_trusted
                or not self.transport.session.is_active or (callable(floor) and floor())
                or epoch in self._uncertain_recovery_epochs):
            return
        self._uncertain_recovery_epochs.add(epoch)
        self._uncertain_recovery_epochs = {value for value in self._uncertain_recovery_epochs if value >= epoch-64}
        text = "Pardon, pouvez-vous répéter ?" if self.policy.reply_language.lower().startswith("fr") else "Sorry, could you repeat that?"
        spoken, _, response_id = await self.policy.finalize_response_with_identity(text, response_kind="repair")
        if self.policy.is_stale(epoch) or not self.transport.session.is_active or not spoken:
            self.policy.discard_pending_playback(response_id)
            return
        frame = TTSSpeakFrame(spoken, append_to_context=True)
        frame.metadata[SPEECH_TURN_EPOCH] = epoch
        frame.metadata[SPEECH_RESPONSE_ID] = response_id
        await self.response_policy.queue_frame(frame, FrameDirection.DOWNSTREAM)
        await self._emit({"type":"uncertain_transcription_repair", "turn_epoch":epoch, "response_id":response_id})

    async def _recover_untranscribed_turn(self) -> None:
        """Ask once after confirmed speech yielded no recognizable words."""
        now = time.monotonic()
        if now - self._last_turn_recovery_at < 10.0:
            return
        self._last_turn_recovery_at = now
        epoch = self.policy.turn_epoch
        french = self.policy.reply_language.lower().startswith("fr")
        text = "Pardon, pouvez-vous répéter ?" if french else "Sorry, could you repeat that?"
        spoken, _, response_id = await self.policy.finalize_response_with_identity(text, response_kind="repair")
    ...
    def _register_handlers(self) -> None:
        ...
        async def on_pipeline_error(_worker, frame) -> None:
            if frame.fatal:
                self._startup_error = frame.error
    ...
    async def _resolve_repetition_retry(self, epoch: int) -> None:
        if epoch != self._repeat_retry_epoch or self._repeat_retry_message is None:
            return
        # The correction belongs to this draft only. Leaving it in context
        # keeps telling the model that an old reply is still being retried on
        # later customer turns.
        with contextlib.suppress(ValueError):
    ...
    async def _speak_tool_preamble(self, name: str) -> None:
        ...
        if not getattr(self.policy, "last_caller_transcript_trusted", True):
            return
        epoch = self.policy.turn_epoch
        french = self.policy.reply_language.lower().startswith("fr")
        if name.startswith("whatsapp_") and ("send" in name or "reply" in name):
            line = "Je vous envoie cela sur WhatsApp." if french else "I'll send that on WhatsApp."
        elif name.startswith("business_"):
            line = "Je m'en occupe." if french else "I'll take care of that."
        else:
            line = "Je vérifie cela." if french else "I'll check that."
        spoken, _, response_id = await self.policy.finalize_response_with_identity(line, response_kind="progress")
        if not spoken or self.policy.is_stale(epoch):
            self.policy.discard_pending_playback(response_id)
            return
        frame = TTSSpeakFrame(spoken, append_to_context=False)
        frame.metadata[SPEECH_TURN_EPOCH] = epoch
        frame.metadata[SPEECH_RESPONSE_ID] = response_id
        # Queue downstream of the processor currently awaiting the tool. The
        # response policy and final audio guard still enforce turn permission.
        # Sending this to worker.queue_frame puts it behind its own tool wait.
        await self.response_policy.queue_frame(frame, FrameDirection.DOWNSTREAM)
    ...
    async def _attach_tools(self) -> None:
        ...
        if not catalog:
            ...
            return
        self.tools.set_catalog_changed_handler(self._refresh_tool_context)
        await self._refresh_tool_context()

    async def _refresh_tool_context(self) -> None:
        """Publish the actual catalog after initial attachment or a live reload."""
        if self.speculative_turn is not None:
            await self.speculative_turn.cancel("tool_catalog_changed")
        ...
        existing = self._tool_protocol_message
        if existing is not None:
            existing["content"] = instructions or "No executable tools are connected."
        elif instructions:
            self._tool_protocol_message = {"role": "system", "content": instructions}
            self.context.add_message(self._tool_protocol_message)
    ...
    async def start(self, timeout_secs: float = 20.0) -> None:
        ...
        failed = asyncio.create_task(self._startup_failed.wait())
        try:
            completed, _ = await asyncio.wait(
                {ready, failed, self._runner_task}, timeout=timeout_secs,
                return_when=asyncio.FIRST_COMPLETED,
            )
            if self._startup_failed.is_set():
                raise RuntimeError(self._startup_error or "Voice provider startup failed")
            if self._runner_task in completed:
                self._runner_task.result()
                raise RuntimeError("Voice worker stopped before startup completed")
            if ready not in completed:
                raise TimeoutError("Voice pipeline did not become ready")
