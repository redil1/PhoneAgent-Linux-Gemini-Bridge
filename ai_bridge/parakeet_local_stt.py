"""Local Parakeet/Whisper decoders and the legacy buffered STT service.

Model work runs on one owned inference thread. The Apple GPU Whisper path uses
LocalNeuralSTTService for shared Silero/Smart Turn/floor control; other legacy
local providers still use the buffered service below. Decoding cost depends on
the model, hardware and confidence settings and must be measured on the host.
"""
...
_WHISPER_HOTWORDS = (
    "customer support appointment order invoice account reference callback quotation "
    "product service package price date time email address phone number"
)
...
def load_model(model_id: str = DEFAULT_MODEL) -> Any:
    ...
    with _MODEL_LOCK:
        ...
        if cached is not None:
            return cached
        if "mlx-community" in model_id and "whisper" in model_id:
            from .mlx_whisper_decoder import MLXWhisperDecoder
            decoder = MLXWhisperDecoder(model_id)
            decoder.prewarm()
            _MODEL_CACHE[model_id] = decoder
            return decoder
        wants_whisper = any(
        ...
        try:
            ...
            # CTranslate2 owns this decoder; PyTorch is not a dependency of
            # faster-whisper and must not prevent CPU-only installations.
            device = "cuda" if ctranslate2.get_cuda_device_count() else "cpu"
            supported = ctranslate2.get_supported_compute_types(device)
            comp_type = "float16" if device == "cuda" else (
            ...
            _MODEL_CACHE[model_id] = model
            logger.info(
                "loaded local faster-whisper model=%s elapsed_ms=%.1f",
                whisper_model,
                (time.perf_counter() - started) * 1000,
            )
            return model
...
def _decode_whisper(
    ...
    *,
    ...
    return evaluate_whisper_segments(
        ...
        diagnostics={"engine": "faster-whisper", "model": getattr(model, "model_size_or_path", None), "beam_size": beam_size},
    )


def evaluate_whisper_segments(
    segments: list[Any], samples: NDArray[np.float32], *, language: str | None,
    diagnostics: dict[str, Any],
) -> STTHypothesis:
    """Apply the same acoustic confidence gate to local Whisper decoders."""
...
def transcribe_pcm(
    ...
    *, initial_prompt: str | None = None,
    ...
    if getattr(model, "backend_kind", None) == "mlx-whisper":
        if initial_prompt:
            return model.decode(samples, language=language, initial_prompt=initial_prompt)
        return model.decode(samples, language=language)
    if initial_prompt:
        raise ValueError("Vocabulary hints are currently supported only by the MLX Whisper decoder")
...
async def load_model_async(model_id: str = DEFAULT_MODEL) -> Any:
    """Load on the same owned executor that will perform live inference."""
    return await asyncio.get_running_loop().run_in_executor(_inference_executor(), load_model, model_id)
...
async def transcribe_pcm_async(
    ...
    *, initial_prompt: str | None = None,
    ...
    return await loop.run_in_executor(
        _inference_executor(), lambda: transcribe_pcm(pcm, model_id, language, initial_prompt=initial_prompt) if initial_prompt else transcribe_pcm(pcm, model_id, language)
    )
