...
from .consent import negated_or_reported
...
class GroundedToolArguments:
    ...
    blocked: bool = False
    reason: str = "literal_confirmation_required"
        ...
        return json.dumps(
            {
                ...
                "error": self.reason,
                "message": (
                    "Only use contact details explicitly provided by the caller. Omit unknown optional fields; the current phone number can be registered without them."
                    if self.reason == "contact_details_not_grounded" else
...
def ground_tool_arguments(
    ...
    *,
    ...
    if tool_name == "business_upsert_current_lead":
        markers = {
            "name": r"(?:my name is|je m'appelle|mon nom est)\s+",
            "email": r"(?:my (?:email|email address) is|email me at|mon (?:email|courriel) est|mon adresse (?:email|courriel) est)\s+",
            "company": r"(?:my company is|i work (?:at|for)|mon entreprise est|je travaille (?:chez|pour))\s+",
            "notes": r"(?:please note|note that|with the note|veuillez noter|notez que)\s+",
        }
        blocked = []
        changed = []
        for field, marker in markers.items():
            proposed = str(arguments.get(field) or "").strip()
            if not proposed:
                continue
            bound = None
            for text, trusted in history:
                cleaned = text.replace("\u2019", "'").strip()
                match = re.search(marker + r"(.+?)(?=,|[.!?](?:\s|$)|\s+(?:and|et)\s+|$)", cleaned, re.IGNORECASE)
                if match:
                    outside_literal = cleaned[:match.start()] + " " + cleaned[match.end():]
            ...
            elif proposed != bound[0]:
                arguments[field] = bound[0]
                changed.append(field)
        if blocked:
            return GroundedToolArguments(raw_arguments, tuple(blocked), True, "contact_details_not_grounded")
        return GroundedToolArguments(json.dumps(arguments, ensure_ascii=False), tuple(changed))
