...
logger = logging.getLogger("PhoneAgentOpenWA")
...
class OpenWAConfig(BaseModel):
    ...
    # Send acceptance is the synchronous result. Device delivery continues on
    # the event bridge; keeping the voice turn blocked cannot make it arrive.
    delivery_confirmation_timeout_ms: int = Field(default=0, ge=0, le=10_000)
...
class OpenWAClient:
    def __init__(self, config: OpenWAConfig, session: aiohttp.ClientSession, event_sink: Any = None) -> None:
        self.config = config
    ...
    async def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        ...
        operation = (
            "contact_lookup" if "/contacts/check/" in path else
            path.rsplit("/", 1)[-1] if "/messages/" in path else
            "history" if path.endswith("/messages") else "session_status"
        )
        outcome = "ok"
        try:
            ...
            await emit_stage_latency(self._event_sink, "openwa", operation, started, outcome=outcome)
...
class OpenWAToolRuntime:
    def __init__(
        ...
        *,
        ...
        self._chat_confirmations: dict[str, str] = {}
    ...
    async def start(self) -> dict[str, RealtimeTool]:
        ...
        self.client = OpenWAClient(self.config, self.session, event_sink=self._emit)
        is_reachable = False
        try:
            ...
            logger.warning(
                "OpenWA companion at %s is unreachable (%s); OpenWA tools will not be registered",
                self.config.base_url,
                exc,
            )
            await self._emit(
            ...
            # Server responded (e.g. HTTP response without status endpoint)
            is_reachable = True
            await self._emit(
                {
                    "type": "openwa_account_context_unavailable",
        ...
        await self._emit(
            {
                ...
                "state": "ready" if is_reachable else "unavailable",
    ...
    async def _current_chat(self) -> str:
        ...
        phone = self._phone_digits
        if not phone:
            raise OpenWAError(
                "The authenticated current-call recipient is unavailable. Messaging is unavailable until call metadata is restored."
            )
    ...
    async def _execute(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "whatsapp_last_delivery_status":
            ...
            statuses = {**statuses, **self._chat_confirmations}
    ...
    async def _sent(
        ...
        *,
        ...
        if message_id and self.event_bridge is not None:
            ...
            if not self._self_chat and self.config.delivery_confirmation_timeout_ms > 0:
        ...
        if chat_confirmed:
            ...
            guidance = (
                "WhatsApp accepted the message request. You may say it was sent to WhatsApp, "
                "but delivery has not been confirmed. Do not promise it will appear shortly "
                "or claim it was delivered or read."
            )
        if message_id and chat_id and self._self_chat and self.config.delivery_confirmation_timeout_ms == 0:
            task = asyncio.create_task(self._confirm_chat_in_background(chat_id, message_id))
    ...
    async def _confirm_chat_in_background(self, chat_id: str, message_id: str) -> None:
        try:
            if await self._confirm_self_chat_history(chat_id, message_id, budget_seconds=1.5):
                self._chat_confirmations[message_id] = "confirmed_in_chat"
                if len(self._chat_confirmations) > 256:
                    self._chat_confirmations.pop(next(iter(self._chat_confirmations)))
                await self._emit({
                    "type": "openwa_delivery_status", "message_id": message_id,
                    "status": "confirmed_in_chat", "delivery_confirmed": False,
                })
        except Exception:
            logger.debug("Background WhatsApp chat confirmation unavailable")

    async def _confirm_self_chat_history(self, chat_id: str, message_id: str, *, budget_seconds: float | None = None) -> bool:
        client = self._require_client()
        if budget_seconds is None:
            budget_seconds = min(self.config.delivery_confirmation_timeout_ms / 1_000, 1.5)
    ...
    def _definitions() -> dict[str, dict[str, Any]]:
        ...
        return {
            "whatsapp_read_current_customer_chat": tool(
                "Read recent WhatsApp messages from the customer on the current phone call.",
                {
            ...
            "whatsapp_send_text_current_customer": tool(
                ...
                "You must preserve every word exactly when sending requested plans, links, or customer details. "
                "Compose the text from verified facts.",
                {"text": text},
                ["text"],
            ),
