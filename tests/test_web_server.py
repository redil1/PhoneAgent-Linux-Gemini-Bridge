...
def test_persona_task_and_child_configuration_apply_end_to_end(tmp_path: Path) -> None:
    async def _test() -> None:
        ...
        async with TestClient(TestServer(server.app)) as client:
            ...
            live_memory = server._active_caller_memory()
...
def test_task_contract_can_be_authored_and_deleted(tmp_path: Path) -> None:
    ...
    assert "general_conversation" in shipped
    ...
    saved = engine.save_contract(
        {
            ...
            "opening_greeting": {"fr": "Bonjour, ici PhoneAgent."},
            "spoken_max_words": 30,
    ...
    assert engine.is_user_authored("general_conversation") is False
...
def test_shipped_task_cannot_be_deleted(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="not a Studio-authored task"):
        engine.delete_contract("general_conversation")
    ...
    engine.save_contract(
        {
            ...
            "title": "My Edited General Task",
            "objective": "Edited objective.",
        }
    )
    ids = [contract["id"] for contract in engine.get_all_contracts()]
    assert ids.count("general_conversation") == 1
    assert engine.require_contract("general_conversation")["title"] == "My Edited General Task"
...
@pytest.mark.parametrize("field,stale_value", [
    ("stt_provider", "distil_whisper"),
    ("antigravity_live_endpoint_ms", 777),
    ("smart_turn_completion_threshold", 0.83),
    ("edge_tts_pitch", "+7Hz"),
    ("host_session", {"sample_rate": 8000}),
])
def test_mismatched_voice_host_ready_handshake_terminates_host(field, stale_value) -> None:
    async def _test() -> None:
        server = _studio()
...
def test_invalid_host_settings_make_a_resident_worker_unavailable(monkeypatch) -> None:
    server = _studio()
    _mark_resident_host_current(server)
    monkeypatch.setenv("PHONE_AGENT_LINK_KEY_BASE64", "invalid-base64")
    assert server._resident_host_matches_current_config() is False
