...
async def test_greeting_is_deterministic_and_queued_once() -> None:
    ...
    class FakePolicy:
        ...
        async def finalize_response_with_identity(self, text: str, *, response_kind: str = "turn"):
            finalized.append((text, response_kind))
            return text, object(), "greeting-test-response"
...
def test_voice_host_ready_reports_effective_parsed_configuration(
    ...
    monkeypatch.setenv("PHONE_AGENT_ANTIGRAVITY_ENDPOINT_MS", "750")
    monkeypatch.setenv("PHONE_AGENT_SMART_TURN_COMPLETION_THRESHOLD", "0.83")
    monkeypatch.setenv("PHONE_AGENT_EDGE_TTS_VOLUME", "-12%")
    monkeypatch.setenv("PHONE_AGENT_EDGE_TTS_PITCH", "+4Hz")
    ...
    assert reported["antigravity_live_endpoint_ms"] == 750
    assert reported["smart_turn_completion_threshold"] == 0.83
    assert reported["edge_tts_volume"] == "-12%"
    assert reported["edge_tts_pitch"] == "+4Hz"
    assert len(str(reported["system_prompt_sha256"])) == 64


def test_parsed_host_state_and_transport_queue_match_without_exposing_keys(monkeypatch):
    import json

    monkeypatch.setenv("PHONE_AGENT_INPUT_QUEUE_FRAMES", "9")
    monkeypatch.setenv("PHONE_AGENT_DEVICE_ID", "private-fixture-device")
    agent = _commanded_host(monkeypatch)
    host = agent.config.host_session_state()
    params = agent._transport_params()
    assert params.input_queue_frames == host["input_queue_frames"] == 9
    assert params.frame_ms == host["frame_ms"] == 20
    assert params.audio_in_sample_rate == params.audio_out_sample_rate == host["sample_rate"] == 16000
    assert "private-fixture-device" not in json.dumps(host)
    assert "AAAAAAAAAAAAAAAA" not in json.dumps(host)
    assert len(host["link_key_sha256"]) == 64


@pytest.mark.parametrize("name,value", [("PHONE_AGENT_SAMPLE_RATE", "8000"), ("PHONE_AGENT_FRAME_MS", "10")])
def test_unqualified_frame_geometry_is_rejected_before_host_start(monkeypatch, name, value):
    monkeypatch.setenv(name, value)
    with pytest.raises(ValueError, match="qualified phone transport"):
        _commanded_host(monkeypatch)


def test_pairing_material_is_bounded_in_both_session_input_forms(monkeypatch):
    import base64

    from phone_agent_gateway.ai_bridge.runtime_config import ProviderConfig

    environment = {"PHONE_AGENT_LINK_KEY_BASE64": base64.b64encode(b"x" * 4097).decode()}
    with pytest.raises(ValueError, match="between 32 and 4096"):
        RuntimeConfig.from_session_environment(environment, providers=ProviderConfig())
