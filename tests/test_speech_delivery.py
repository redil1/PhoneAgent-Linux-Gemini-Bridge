...
def test_text_normalizer_expands_currency_without_domain_vocabulary() -> None:
    normalizer = UniversalTextNormalizer()
    text = "The plan costs $25 per month."
    normalized = normalizer.normalize(text)
    assert normalized == "The plan costs 25 dollars per month."


def test_text_normalizer_accepts_package_supplied_pronunciation() -> None:
    normalizer = UniversalTextNormalizer({"ACME": "Ack me"})
    assert normalizer.normalize("Call ACME.") == "Call Ack me."
