...
def test_registration_cannot_invent_optional_contact_details():
    result = ground_tool_arguments("business_upsert_current_lead", '{"name":"Invented Name","email":"invented@example.com"}',
                                   "Can you register me in the database?")
    assert result.blocked
    assert set(result.grounded_fields) == {"name", "email"}
    assert json.loads(result.blocked_output())["error"] == "contact_details_not_grounded"
    assert not ground_tool_arguments("business_upsert_current_lead", "{}", "Register me, please.").blocked


def test_registration_uses_trusted_caller_details_without_turning_names_into_consent_words():
    result = ground_tool_arguments("business_upsert_current_lead", '{"name":"Said","email":"said@example.com"}',
                                   "Register me, please.", caller_turns=[("My name is Said.", True),
                                   ("My email is said@example.com.", True), ("Register me, please.", True)])
    assert not result.blocked
    assert json.loads(result.raw_arguments)["email"] == "said@example.com"


def test_registration_rejects_uncertain_or_withdrawn_details():
    for history in [ [("My name is Ada.", False), ("Register me.", True)],
                     [("My name is Ada.", True), ("Do not use Ada.", True), ("Register me.", True)]]:
        result = ground_tool_arguments("business_upsert_current_lead", '{"name":"Ada"}', "Register me.", caller_turns=history)
        assert result.blocked
