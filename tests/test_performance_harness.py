...
def contract_profile():
    # Exercise the same deterministic stage contract on a genuinely matching
    # registered host profile. Hardware qualification checks remain enabled.
    key = (platform.system().lower(), platform.machine().lower())
    profiles = {
        ("linux", "x86_64"): "linux-x86_64-contract-ci",
        ("darwin", "arm64"): "macos-arm64-contract-ci",
    }
    if key not in profiles:
        pytest.skip("No deterministic contract profile is registered for this host")
    return load_profile(DEFAULT_PROFILES, profiles[key])
...
@pytest.mark.asyncio
async def test_contract_profile_measures_all_stages_and_sixty_turn_drift(contract_profile) -> None:
...
@pytest.mark.asyncio
async def test_threshold_regression_is_machine_visible(contract_profile) -> None:
    profile = contract_profile
...
@pytest.mark.asyncio
async def test_contract_profile_still_rejects_the_wrong_host(contract_profile):
    contract_profile["environment"]["os"] = "a-different-operating-system"
    with pytest.raises(PerformanceHarnessError, match="host OS"):
        await run_benchmark(contract_profile, corpus=DEFAULT_CORPUS)
