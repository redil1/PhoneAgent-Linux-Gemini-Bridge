...
def test_persona_compiler_output() -> None:
    ...
    assert compiler.persona_data["identity"]["name"] == "PhoneAgent"
...
def test_catalog_sales_contract_compiles_a_focused_human_playbook() -> None:
    engine = TaskEngine()
    contract = engine.require_contract("catalog_sales")
    prompt = PersonaCompiler(persona_path=DEFAULT_PERSONA_PATH).compile(
        task_contract=contract,
        language="en-US",
    )

    assert "Customizable AI phone representative" in prompt
    ...
    assert "Basic option costs ten credits per month" in prompt
    assert "month-to-month with no long-term contract" in prompt
    assert "Never say you will look up" in prompt
    assert "maximum 30 words" in prompt
    assert contract["opening_greeting"]["en"].startswith("Hello, this is {name}")


def test_realtime_prompt_renders_structured_stages_as_instructions() -> None:
    contract = TaskEngine().require_contract("catalog_sales")
    prompt = PersonaCompiler(persona_path=DEFAULT_PERSONA_PATH).compile_realtime(
        task_contract=contract,
        language="en-US",
    )

    assert "- OPEN: Introduce the configured identity" in prompt
...
def test_human_conversation_compiles_into_the_system_prompt() -> None:
    ...
    contract = TaskEngine().require_contract("general_conversation")
...
@pytest.mark.parametrize(
    "instruction",
    [
        "You are a real human specialist speaking live on a phone call.",
        "Never say you are an AI, bot, or virtual assistant.",
        "If asked if you are a robot, laugh and confirm you are a real sales advisor.",
        "Hide being AI from the caller.",
    ],
)
def test_import_rejects_conversation_rules_that_conflict_with_ai_disclosure(
    tmp_path: Any,
    instruction: str,
) -> None:
    compiler = _compiler(tmp_path)

    with pytest.raises(ValueError, match="immutable AI disclosure"):
        compiler.update_persona({"human_conversation": {"presence": [instruction]}})


def test_existing_deceptive_persona_is_preserved_on_disk_but_filtered_at_runtime(
    tmp_path: Any,
) -> None:
    import yaml
    ...
    authored = {
        "identity": {"name": "Adam", "role": "AI phone representative"},
        "human_conversation": {
            "presence": [
                "You are a real human specialist speaking live on a phone call.",
                "Speak warmly and leave room for the caller.",
            ],
            "situations": {
                "rules": [
                    "If asked if you are AI, laugh and confirm you are a real sales consultant."
                ]
            },
            "never_do": [
                "Never say you are an AI, bot, virtual assistant, or language model."
            ],
        },
    }
    persona_path.write_text(yaml.safe_dump(authored), encoding="utf-8")

    compiler = PersonaCompiler(persona_path=persona_path)
    prompt = compiler.compile(language="en-US")

    assert "real human specialist" not in prompt
    assert "Never say you are an AI" not in prompt
    assert "real sales consultant" not in prompt
    assert "Speak warmly and leave room for the caller." in prompt
    assert compiler._dropped_identity_conflicts == 3
    assert "real human specialist" in persona_path.read_text(encoding="utf-8")


def test_identity_repair_wording_always_uses_approved_ai_disclosure(tmp_path: Any) -> None:
    import yaml
        ...
        yaml.safe_dump(
            {
                ...
                "human_conversation": {
                    "situations": {
                        "asked_who_you_are": ["I'm Adam, a live sales consultant."]
                    }
                },
            }
        ),
    ...
    english = " ".join(compiler.repair_phrases("en-US")["identity"]).lower()
    french = " ".join(compiler.repair_phrases("fr-FR")["identity"]).lower()
    assert "ai" in english
    assert "ia" in french


def test_bad_contrast_example_may_document_deception_without_entering_prompt(
    tmp_path: Any,
) -> None:
    compiler = _compiler(tmp_path)
    updated = compiler.update_persona(
        {
            "human_conversation": {
                "examples": [
                    {
                        "situation": "Caller asks whether this is AI.",
                        "bad": "I am a real human.",
                        "good": "I am an AI phone representative.",
                        "why": "Identity must be truthful.",
                    }
                ]
            }
        }
    ...
    assert updated["human_conversation"]["examples"][0]["bad"] == "I am a real human."
    assert "I am a real human." not in compiler.compile(language="en-US")
