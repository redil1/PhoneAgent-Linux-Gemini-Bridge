...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Are you a human or an AI?", False, TurnQuality.IDENTITY_CHALLENGE),
        ("Be honest, are you a robot?", False, TurnQuality.IDENTITY_CHALLENGE),
        (
            "Vous êtes humain ou une intelligence artificielle ?",
            False,
            TurnQuality.IDENTITY_CHALLENGE,
        ),
        ("Soyez honnête, vous êtes un robot ?", False, TurnQuality.IDENTITY_CHALLENGE),
        # Noise.
...
async def test_exact_last_call_rehearsal_can_repeat_the_requested_intro(
    ...
    runtime = _runtime(tmp_path)
    intro = "I'm PhoneAgent, the AI representative, calling about the configured service."
    ...
    assert repeated.startswith("I'm PhoneAgent, the AI representative")
    assert "configured service" in repeated
...
def test_a_reply_superseded_by_a_newer_turn_is_never_spoken(tmp_path: Any) -> None:
    ...
    async def capture(frame: Any, direction: Any = None) -> None:
        if isinstance(frame, LLMTextFrame):
            spoken.append(frame.text.rstrip())
...
def test_exhausted_model_repeat_recovery_never_leaves_the_phone_silent(
    ...
    spoken = [frame.text.rstrip() for frame in forwarded if isinstance(frame, LLMTextFrame)]
    assert len(spoken) == 1
...
def test_duplicate_lead_in_is_dropped_but_new_answer_is_spoken(tmp_path: Any) -> None:
    ...
    runtime = _runtime(tmp_path)
    runtime.available_tools.add("send_sms")
