...
def test_slots_fill_from_the_callers_own_words() -> None:
    ...
    actions = runtime.observe_caller_turn("Yes, go ahead.", consent_scopes=frozenset({'continue'}))

    assert actions.state_delta == {"permission": "Yes, go ahead."}
    assert runtime.state["permission"] == "Yes, go ahead."
...
def test_the_brief_names_exactly_one_next_question() -> None:
    runtime = TaskRuntime(CONTRACT)
    runtime.observe_caller_turn("Yes, sure.", consent_scopes=frozenset({'continue'}))
    brief = runtime.brief()
...
def test_a_second_observation_does_not_refill_a_slot() -> None:
    runtime = TaskRuntime(CONTRACT)
    first = runtime.observe_caller_turn("Yes.", consent_scopes=frozenset({'continue'}))
...
def test_stage_advances_only_when_its_requirements_are_met() -> None:
    ...
    advanced = runtime.observe_caller_turn("Yes, go ahead.", consent_scopes=frozenset({'continue'}))
...
def test_structured_stages_exclude_labelled_policy_rules() -> None:
    ...
    runtime.observe_caller_turn("Yes, go ahead.", consent_scopes=frozenset({'continue'}))
    assert runtime.stage == "DISCOVER"
...
def test_summary_reports_what_was_collected() -> None:
    ...
    assert set(summary["collected"]) == {"content", "budget"}
    assert summary["missing"] == ['permission']
...
def test_shipped_catalog_contract_declares_slots_and_facts() -> None:
    ...
    assert runtime.observe_caller_turn("Je préfère Service Alpha.").state_delta


def test_shipped_catalog_contract_recognizes_real_call_answers() -> None:
    runtime = TaskRuntime(TaskEngine().require_contract("catalog_sales"))

    runtime.record("permission_to_continue", "granted")
    preference = runtime.observe_caller_turn("I mostly need Service Alpha.")
    budget = runtime.observe_caller_turn("I want to stay under 20.")
    runtime.observe_caller_turn("I'm open if it fits.")

    assert preference.state_delta["product_preferences"] == "Service Alpha"
