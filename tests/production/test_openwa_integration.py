...
async def test_self_chat_uses_history_confirmation_without_claiming_delivery() -> None:
    ...
    await asyncio.gather(*tuple(runtime._confirmation_tasks))
    assert runtime._chat_confirmations["self-message"] == "confirmed_in_chat"
    assert result["delivery_confirmed"] is False
    assert result["confirmation_status"] == "accepted"
    assert "delivery has not been confirmed" in result["guidance"]
