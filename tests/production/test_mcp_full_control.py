...
def test_the_full_configuration_surface_is_reachable(calls) -> None:
    ...
    mcp_server._call_tool("phone_agent_delete_skill", {"name": "s"})
...
def test_each_business_integration_is_reachable(integration: str, calls) -> None:
    ...
    assert calls[1][2] == {'config': {'a': 1}}


def test_existing_enveloped_integration_clients_are_not_double_wrapped(calls):
    mcp_server._call_tool('phone_agent_set_integration',
                         {'integration': 'openwa', 'config': {'config': {'enabled': False}}})
    assert calls[0][2] == {'config': {'enabled': False}}
