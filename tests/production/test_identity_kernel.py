...
def test_agent_package_revision_can_replace_the_persona_identity_id(tmp_path: Path) -> None:
    kernel = _kernel(tmp_path)
    candidate = kernel.active.model_copy(deep=True)
    candidate.identity_id = "configured-agent"
    candidate.core.name = "Configured Agent"
    revision = kernel.create_revision(
        candidate.model_dump(mode="json"),
        reason="Replace the configured persona",
        actor="mcp_package",
    )

    evaluated = kernel.evaluate_revision(revision.revision_id)
    assert evaluated.evaluation is not None and evaluated.evaluation.passed
    kernel.approve_revision(revision.revision_id, actor="mcp_package")
    activated = kernel.activate_revision(revision.revision_id)

    assert activated.identity_id == "configured-agent"
    assert activated.core.name == "Configured Agent"
