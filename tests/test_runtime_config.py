...
def test_sensevoice_french_falls_back_to_reliable_whisper() -> None:
    ...
    assert isinstance(services.stt, LocalNeuralSTTService)
...
def test_default_voice_stack_is_free_edge_tts(
    ...
    assert config.tts_model == "edge-online-neural"
    assert config.tts_voice_id == "en-US-AvaNeural"


def test_supertonic_configuration_when_requested(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_provider_change_defaults_are_consistent_and_preserve_explicit_choices() -> None:
    from dataclasses import replace
    config = ProviderConfig(tts_provider='edge_tts',tts_model='edge-online-neural',tts_voice_id='en-US-AvaNeural',tts_aggregation='phrase')
    changed = config.provider_change_defaults({'tts_provider':'supertonic'})
    assert changed['tts_aggregation']=='sentence'
    assert changed['tts_model']=='supertonic-2'
    assert changed['tts_voice_id']=='M1'
    replace(config,**changed).validate(require_credentials=False)
    explicit = config.provider_change_defaults({'tts_provider':'supertonic','tts_voice_id':'F2','tts_aggregation':'sentence'})
    assert explicit['tts_voice_id']=='F2'
    assert config.provider_change_defaults({'tts_provider':'edge_tts'})=={'tts_provider':'edge_tts'}
    assert config.tts_voice_id=='en-US-AvaNeural'


def test_switching_back_to_cloud_stt_cannot_keep_a_whisper_model_label() -> None:
    config=ProviderConfig(stt_provider='whisper_turbo',stt_model='large-v3-turbo')
    changed=config.provider_change_defaults({'stt_provider':'antigravity_live'})
    assert changed['stt_model']=='gemini-3.1-flash-live-preview'


def test_an_explicit_incompatible_aggregation_is_not_silently_rewritten() -> None:
    from dataclasses import replace
    config=ProviderConfig(tts_provider='edge_tts',tts_aggregation='phrase')
    changed=config.provider_change_defaults({'tts_provider':'supertonic','tts_aggregation':'phrase'})
    with pytest.raises(ValueError,match='supported only by edge_tts'):
        replace(config,**changed).validate(require_credentials=False)


@pytest.mark.asyncio
async def test_local_decoder_budget_and_committed_stop_strategy_reach_the_aggregator():
    from types import SimpleNamespace

    from phone_agent_gateway.ai_bridge.committed_turn_strategy import (
        CommittedTranscriptStopStrategy,
    )
    ...
    config=SimpleNamespace(providers=ProviderConfig(stt_provider='whisper_turbo',tts_provider='edge_tts'),
                           sample_rate=16000,task_id='customer_support',memory_enabled=False,system_prompt='')
    pipeline=ProductionCallPipeline(PhoneAgentTransport(PhoneAgentTransportParams()),config,caller_id='unknown:budget-test')
    try:
        assert pipeline.user_aggregator._params.user_turn_stop_timeout==pipeline.services.stt.turn_stop_watchdog_timeout_secs
        assert pipeline.user_aggregator._params.user_turn_stop_timeout>2*pipeline.services.stt._decode_timeout_secs
        assert isinstance(pipeline.user_aggregator._params.user_turn_strategies.stop[0],CommittedTranscriptStopStrategy)
    finally:
