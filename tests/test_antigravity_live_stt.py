...
async def test_antigravity_live_stt_frame_flow() -> None:
    ...
    # This codec fixture represents speech; the PCM bytes themselves are DC,
    # so explicitly provide its acoustic verdict instead of relying on an ASR
    # string to manufacture a speech epoch. Noise rejection is tested separately.
    from pipecat.audio.vad.vad_analyzer import VADState

    async def speech_verdict(_audio):
        return VADState.SPEAKING

    service._vad.analyze_audio = speech_verdict
    ...
    # This fixture ends with a complete envelope. Await that event, not a
    # wall-clock guess that races the worker thread under full-suite load.
    await asyncio.wait_for(asyncio.shield(service._reader_task), timeout=2)

    assert not any(isinstance(f, TranscriptionFrame) for f in pushed_frames)
    assert service._last_transcript == 'testing one two'
    assert service._provider_final_seen
    # Model the final hypothesis remaining stable during the supplied silence.
    service._last_transcript_update_at = time.monotonic() - 1.1
    service._last_audio_at = time.monotonic()
    service._last_speech_at = time.monotonic() - 1.1
    service._received_silence_sec = 1.1
    service._acoustic_active = False
...
async def test_antigravity_live_stt_silence_watchdog_endpointing() -> None:
    service = AntigravityLiveSTTService(
        ...
        partial_transcript_stability_ms=20,
        base_url="https://127.0.0.1:53857",
        csrf_token="test-csrf-token",
    )
    ...
    http_response = (
        ...
        b"Content-Type: application/connect+json\r\n"
        b"Transfer-Encoding: chunked\r\n"
        b"\r\n" + _build_chunked_body([env_ready, env_partial, env_partial])[:-5]
    )

    class LiveSocket(_MockSocket):
        def recv(self, bufsize: int) -> bytes:
            if not self._raw and not self.closed:
                # A healthy idle stream times out; EOF means transport loss.
                time.sleep(0.01)
                raise TimeoutError()
            return super().recv(bufsize)

    mock_sock = LiveSocket(http_response)
    mock_conn = _StreamConn(mock_sock)  # type: ignore[arg-type]

    service._stream = mock_conn
    service._session_id = "session-xyz-456"
    service._speech_epoch = 1  # This protocol fixture represents detected speech.
...
@pytest.mark.asyncio
async def test_provider_partial_without_acoustic_speech_cannot_create_a_turn() -> None:
    service = AntigravityLiveSTTService(
        barge_in_min_ms=250,
        base_url="https://127.0.0.1:53857",
        csrf_token="test-csrf-token",
...
def test_speculative_endpointing_keeps_extra_grace_for_incomplete_speech() -> None:
    ...
    assert service._required_silence() == pytest.approx(0.9)
    service._last_transcript = "I need help because"
    assert service._required_silence() == pytest.approx(3.0)


@pytest.mark.parametrize('text', ['Yes, yes, it seems very', 'Oui, ça semble très'])
def test_complete_prosody_cannot_release_an_unfinished_evaluation(text):
    service = AntigravityLiveSTTService(
        speculative_pipeline_enabled=True, incomplete_endpoint_ms=3000,
        base_url='https://127.0.0.1:53857', csrf_token='test-token',
    )
    service._stage_transcription(text, is_final=True)
    service._smart_turn_incomplete = False
    assert service._required_silence() == pytest.approx(3.0)
    # A continuation changes the semantic result without lowering the normal
    # acoustic/partial-stability requirements.
    service._last_transcript = text + (' expensive.' if 'seems' in text else ' cher.')
    assert service._required_silence() < 3.0
...
def test_correct_english_final_replaces_cross_language_hypothesis() -> None:
    ...
    assert service._required_silence() == pytest.approx(service._fallback_endpoint_sec)
...
@pytest.mark.asyncio
async def test_smart_turn_acoustic_prosody_incomplete_extends_endpoint_grace() -> None:
    service = AntigravityLiveSTTService(
        speculative_pipeline_enabled=True,
        speculative_fast_endpoint_ms=300,
        speculative_incomplete_endpoint_ms=1400,
        smart_turn_enabled=True,
        base_url="https://127.0.0.1:53857",
        csrf_token="test-csrf-token",
    )
    # Transcript text looks grammatically complete
    service._stage_transcription("I want movies", is_final=True)
    assert service._required_silence() == pytest.approx(0.9)

    # When Smart Turn flags acoustic prosody as incomplete (e.g. rising pitch / mid-sentence pause)
    service._smart_turn_incomplete = True
    assert service._required_silence() == pytest.approx(service._incomplete_endpoint_sec)

    # Reset upon resuming speech
    service._smart_turn_incomplete = False
    assert service._required_silence() == pytest.approx(0.9)
    await service._close_session()
...
@pytest.mark.asyncio
async def test_smart_turn_concise_terminal_turn_waits_for_acoustic_evidence() -> None:
    service = AntigravityLiveSTTService(
        speculative_pipeline_enabled=True,
        speculative_fast_endpoint_ms=300,
        speculative_incomplete_endpoint_ms=1400,
    ...
    # Short words are still provisional: "yes ... but" must retain completion patience.
    service._stage_transcription("Yes", is_final=True)
    service._smart_turn_incomplete = False
    assert service._required_silence() == pytest.approx(0.9)
    await service._close_session()
...
@pytest.mark.asyncio
async def test_bot_speaking_barge_in_broadcasts_interruption() -> None:
    ...
    service.broadcast_interruption = mock_broadcast_interruption

    await service.process_frame(BotStartedSpeakingFrame(), FrameDirection.DOWNSTREAM)
    assert service._bot_speaking is True

    service._speech_candidate_at = time.monotonic() - 0.5
    await service._ensure_user_started(force=True)
...
@pytest.mark.asyncio
async def test_bot_taking_floor_resets_streaming_vad_once_at_safe_boundary() -> None:
    from pipecat.frames.frames import BotStartedSpeakingFrame, BotStoppedSpeakingFrame
    ...
    reset = MagicMock(wraps=service._vad.reset_stream)
    service._vad.reset_stream = reset
...
@pytest.mark.asyncio
async def test_incomplete_phrase_uses_incomplete_endpoint_silence() -> None:
    service = AntigravityLiveSTTService(
