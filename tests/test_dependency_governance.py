...
def test_lockfiles_and_cuda_sources_are_explicit() -> None:
    ...
    assert uv["sources"]["torch"] == [
        ...
        {"index": "pypi-torch", "marker": "sys_platform != 'linux'"},
    ]
    indexes = {entry["name"]: entry for entry in uv["index"]}
    assert indexes["pytorch-cu129"] == {
        "name": "pytorch-cu129", "url": "https://download.pytorch.org/whl/cu129", "explicit": True,
    }
    assert indexes["pypi-torch"] == {
        "name": "pypi-torch", "url": "https://pypi.org/simple", "explicit": True,
    }
    # A valid Linux CUDA lock must not make the Mac extra un-installable.
    packages = tomllib.loads((ROOT / "uv.lock").read_text())["package"]
    apple_torch = next(package for package in packages if package["name"] == "torch" and package["version"] == "2.14.0")
    assert any("macosx" in wheel["url"] and "arm64" in wheel["url"] for wheel in apple_torch["wheels"])
