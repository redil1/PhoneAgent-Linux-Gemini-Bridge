...
@pytest.mark.parametrize(
    ...
    [
        ...
        "We are waiting for.",
        "I want to ask about",
        "Hi. Yeah, I wasn't expecting this call. Um to be honest, I'm not really looking to buy",
        "Hi, I wasn't really",
        "I'm not really",
        "It's just",
        "I don't actually",
...
def test_mid_thought_hesitations_and_prepositions_are_incomplete(text: str) -> None:
    assert looks_semantically_incomplete(text)
...
@pytest.mark.parametrize(
    "text",
    [
        "I'm not really looking to buy anything new.",
        "Hi, I wasn't really expecting this. I'm not really looking to buy anything right now.",
        "Hi, I wasn't really expecting this.",
        "Yeah, that's fine.",
        "I already have a satellite dish.",
    ],
)
def test_completed_clauses_are_not_incomplete(text: str) -> None:
    assert not looks_semantically_incomplete(text)



@pytest.mark.parametrize('text', [
    'Yes, I can.', 'No, I cannot.', 'Yes, we do.',
    'Je ne sais pas.', 'Je ne peux pas.', 'Je ne veux pas.',
    'Where are you calling from?', 'What are you waiting for?', 'Who are you speaking with?',
])
def test_complete_ellipsis_and_questions_do_not_force_incomplete_patience(text):
    assert not looks_semantically_incomplete(text)
