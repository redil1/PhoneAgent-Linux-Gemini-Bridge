...
@pytest.mark.asyncio
async def test_fast_tools_do_not_emit_unnecessary_progress_speech() -> None:
...
async def test_missing_required_arguments_produce_guidance_not_a_keyerror() -> None:
    ...
    runtime.policy.last_caller_text = "Send me the offer on WhatsApp."
...
@pytest.mark.asyncio
@pytest.mark.parametrize('name', ['whatsapp_send_text_current_customer', 'business_upsert_current_lead', END_CALL_TOOL_NAME, 'custom_tool'])
async def test_uncertain_latest_audio_blocks_tools_even_without_literal_argument_mismatch(name):
    called = []
    runtime = _runtime_with({name: _tool(name, lambda arguments: called.append(arguments) or {'accepted':True})})
    runtime.policy.last_caller_text = (
        "Envoyez-moi l'offre sur WhatsApp"
        if name == "whatsapp_send_text_current_customer"
        else "à l'abonnement annuel"
    )
    runtime.policy.last_caller_transcript_trusted = False
    result = json.loads(await runtime.execute(name, '{}'))
    assert result['error']=='uncertain_transcription' and result['executed'] is False
    assert not called
    runtime.policy.last_caller_transcript_trusted = True
    await runtime.execute(name, '{}')
    assert len(called)==1


@pytest.mark.asyncio
async def test_whatsapp_send_requires_explicit_current_caller_authorization() -> None:
    called: list[dict[str, Any]] = []
    runtime = _runtime_with(
        {
            "whatsapp_send_text_current_customer": _tool(
                "whatsapp_send_text_current_customer",
                lambda arguments: called.append(arguments) or {"accepted": True},
            )
        }
    )
    runtime.policy.last_caller_text = (
        "I would like the six month plan for thirty nine dollars."
    )
    ...
    assert result["error"] == "explicit_caller_authorization_required"
    assert result["executed"] is False
    assert called == []


@pytest.mark.asyncio
async def test_whatsapp_send_accepts_direct_request_or_yes_to_heard_proposal() -> None:
    called: list[dict[str, Any]] = []
    runtime = _runtime_with(
        {
            "whatsapp_send_text_current_customer": _tool(
                "whatsapp_send_text_current_customer",
                lambda arguments: called.append(arguments) or {"accepted": True, 'message_id': 'm1'},
            )
        }
    )
    runtime.policy.last_caller_text = "Can you send me this offer?"
    assert json.loads(
        await runtime.execute("whatsapp_send_text_current_customer", "{}")
    )["accepted"] is True

    runtime.policy.last_caller_text = "Yes, please."
    runtime.policy._last_ai_response = "Would you like me to send it on WhatsApp?"
    runtime.policy._last_ai_delivery = "completed"
    repeated = json.loads(
        await runtime.execute("whatsapp_send_text_current_customer", "{}")
    )
    assert repeated['duplicate_prevented']
    assert repeated['action_receipt']['state'] == 'accepted'
    assert len(called) == 1


@pytest.mark.asyncio
@pytest.mark.parametrize('event_type', ['tool_started', 'tool_arguments_grounded'])
@pytest.mark.parametrize('change', ['uncertain', 'new_turn'])
async def test_caller_state_is_rechecked_after_awaited_tool_notifications(event_type, change):
    called = []
    name = 'business_create_support_ticket'
    runtime = _runtime_with({name: _tool(name, lambda arguments: called.append(arguments) or {'accepted':True})})
    runtime.policy.turn_epoch = 1
    runtime.policy.last_caller_text = 'Create a support ticket named Mac complete test saying the installation is working.'
    runtime.policy.recent_caller_turns = ((runtime.policy.last_caller_text, True),)
    async def interrupt(event):
        if event['type']==event_type:
            if change=='uncertain':
                ...
                runtime.policy.turn_epoch += 1
    runtime._emit = interrupt
    result = json.loads(await runtime.execute(name, '{"subject":"wrong","description":"wrong"}'))
    assert result['executed'] is False
    assert result['error']==('uncertain_transcription' if change=='uncertain' else 'caller_turn_changed')
    assert not called
